...
public class CcdParseUtils {
    ...
    public static boolean hasContent(ANY element) {
        ...
        if ((element.isSetNullFlavor()) || (!element.hasContent())) return false;
        return true;
    }

    public static boolean hasContent(InfrastructureRoot element) {
        if (element == null) return false;
        if ((element.isNullFlavorDefined()) || (!element.hasContent())) return false;
        if (element instanceof PlanOfCareSection) {
            ...
            return !(CollectionUtils.isEmpty(section.getActs()) && CollectionUtils.isEmpty(section.getEncounters()) &&
                    CollectionUtils.isEmpty(section.getObservations()) && CollectionUtils.isEmpty(section.getProcedures()) &&
                    CollectionUtils.isEmpty(section.getSupplies()) && CollectionUtils.isEmpty(section.getSubstanceAdministrations()));
        }
        return true;
    }

    /**
     * @throws ClassCastException Handle this exception if you think that the provided collection may contain values of an unexpected type.
     */
    public static <E extends ANY, T extends E> T getFirstNotEmptyValue(List<E> values,
        ...
        if (!CollectionUtils.isEmpty(values)) {
            for (E value : values) {
                if (hasContent(value)) return (T) value;
            }
        }
        return null;
    }
    ...
    public static PersonTelecom createTelecom(TEL ccdTelecom, Database database, Person person, String legacyTable) {
    ...
    /**
     * @return Sync Qualifier. By default, 0, which corresponds to {@code PersonTelecomCode.EMAIL}
     */
    ...
    public static OrganizationTelecom createTelecom(TEL ccdTelecom, Database database, Organization org, String legacyTable) {
        OrganizationTelecom orgTelecom = new OrganizationTelecom();
        orgTelecom.setLegacyId(CcdConstants.NWHIN_LEGACY_ID_STR);
        orgTelecom.setDatabase(database);
        orgTelecom.setLegacyTable(legacyTable);

        orgTelecom.setValue(ccdTelecom.getValue());
        orgTelecom.setOrganization(org);
        if (CollectionUtils.isNotEmpty(ccdTelecom.getUses())) {
            TelecommunicationAddressUse telecommunicationAddressUse = ccdTelecom.getUses().get(0);
    ...
    public static PersonAddress createAddress(AD ccdAddress, Database database, Person person, String legacyTable) {
    ...
    public static OrganizationAddress createAddress(AD ccdAddress, Database database, Organization org, String legacyTable) {
        OrganizationAddress organizationAddress = new OrganizationAddress();
        organizationAddress.setLegacyId(CcdConstants.NWHIN_LEGACY_ID_STR);
        organizationAddress.setDatabase(database);
        organizationAddress.setLegacyTable(legacyTable);

        organizationAddress.setOrganization(org);
        if (CollectionUtils.isNotEmpty(ccdAddress.getUses())) {
            PostalAddressUse ccdPostalAddressUse = ccdAddress.getUses().get(0);
            organizationAddress.setPostalAddressUse(ccdPostalAddressUse.getName());
        }
        organizationAddress.setStreetAddress(getTextFromFirstCcdAddressRecord(ccdAddress.getStreetAddressLines()));
        organizationAddress.setCity(getTextFromFirstCcdAddressRecord(ccdAddress.getCities()));
        organizationAddress.setState(getTextFromFirstCcdAddressRecord(ccdAddress.getStates()));
        organizationAddress.setCountry(getTextFromFirstCcdAddressRecord(ccdAddress.getCountries()));
        organizationAddress.setPostalCode(getTextFromFirstCcdAddressRecord(ccdAddress.getPostalCodes()));
        return organizationAddress;
    }

    public static BirthplaceAddress createAddress(AD ccdAddress, Resident resident) {
        if (!CcdParseUtils.hasContent(ccdAddress)) {
            return null;
        }

        final BirthplaceAddress birthplaceAddress = new BirthplaceAddress();
        birthplaceAddress.setResident(resident);
    ...
    public static Name createName(PN ccdName, Database database, Person person, String legacyTable) {
        Name name = new Name();
    ...
    public static Person createPerson(EList<PN> ccdNames, EList<AD> ccdAddresses, EList<TEL> ccdTelecoms,
                                      Database database, String legacyTable, String legacyId) {
        ...
        if (CollectionUtils.isNotEmpty(ccdNames)) {
            for (PN ccdName : ccdNames) {
                if (hasContent(ccdName)) {
                    Name name = createName(ccdName, database, person, legacyTable);
                    names.add(name);
                }
            }
        }
        ...
        if (CollectionUtils.isNotEmpty(ccdAddresses)) {
            for (AD ccdAddress : ccdAddresses) {
                if (hasContent(ccdAddress)) {
                    PersonAddress address = createAddress(ccdAddress, database, person, legacyTable);
        ...
        if (CollectionUtils.isNotEmpty(ccdTelecoms)) {
            ...
            for (TEL ccdTelecom : ccdTelecoms) {
                if (hasContent(ccdTelecom)) {
                    PersonTelecom telecom = createTelecom(ccdTelecom, database, person, legacyTable);
    ...
    public static Person createPerson(ParticipantRole participantRole, Database database, String legacyTable) {
        ...
        return createPerson(participantRole.getPlayingEntity().getNames(), participantRole.getAddrs(),
                participantRole.getTelecoms(), database, legacyTable, legacyId);
    }

    public static Person createPerson(org.eclipse.mdht.uml.cda.Guardian guardian, Database database, String legacyTable) {
        ...
        return createPerson(guardian.getGuardianPerson().getNames(), guardian.getAddrs(),
                guardian.getTelecoms(), database, legacyTable, legacyId);
    }

    public static Organization createOrganization(ON ccdName, EList<AD> ccdAddresses, TEL ccdTelecom, Database database,
                                                  String legacyTable, String legacyId) {
        Organization org = new Organization();
        org.setDatabase(database);
        org.setDatabaseId(database.getId());
        org.setLegacyId(legacyId);
        org.setLegacyTable(legacyTable);

        if (hasContent(ccdName)) {
            org.setName(ccdName.getText());
        }
        ...
        if (CollectionUtils.isNotEmpty(ccdAddresses)) {
            ...
            for (AD ccdAddress : ccdAddresses) {
                if (hasContent(ccdAddress)) {
                    OrganizationAddress address = createAddress(ccdAddress, database, org, legacyTable);
                    addresses.add(address);
                }
            }
        ...
        if (hasContent(ccdTelecom)) {
            org.setTelecom(createTelecom(ccdTelecom, database, org, legacyTable));
        }
    ...
    public static Organization createOrganization(EList<ON> ccdNames, EList<AD> ccdAddresses, EList<TEL> ccdTelecoms,
                                                  Database database, String legacyTable, String legacyId) {
        ...
        if (CollectionUtils.isNotEmpty(ccdTelecoms)) {
            for (TEL ccdTelecom : ccdTelecoms) {
                ...
                if (firstTelecom == null) firstTelecom = ccdTelecom;
            }
            if (ccdTelecomChosen == null) ccdTelecomChosen = firstTelecom;
        }
        return createOrganization(ccdNameChosen, ccdAddresses, ccdTelecomChosen, database, legacyTable, legacyId);
    }
    ...
    private static String[] SUPPORTED_DATE_PATTERNS = {
            "yyyyMMddHHmmss.SSSZ",
            "yyyyMMddhhmmssZ",
            "yyyyMMddhhmmss",
