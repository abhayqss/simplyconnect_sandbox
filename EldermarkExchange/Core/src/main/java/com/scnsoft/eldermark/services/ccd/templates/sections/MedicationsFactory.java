 ...
 */
@Component
public class MedicationsFactory extends RequiredTemplateFactory implements ParsableSectionFactory<MedicationsSection, Medication> {
    ...
    @Override
    public List<Medication> parseSection(Resident resident, MedicationsSection medicationsSection) {
        if (!CcdParseUtils.hasContent(medicationsSection) || CollectionUtils.isEmpty(medicationsSection.getSubstanceAdministrations())) {
        ...
        for (SubstanceAdministration substanceAdministration : medicationsSection.getSubstanceAdministrations()) {
            final Medication medication = sectionEntryParseFactory.parseMedicationActivity(substanceAdministration, resident, LEGACY_TABLE);
            if (medication != null) {
