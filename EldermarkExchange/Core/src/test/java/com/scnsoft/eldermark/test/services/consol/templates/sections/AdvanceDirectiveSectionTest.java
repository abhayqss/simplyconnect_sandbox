...
import com.scnsoft.eldermark.services.consol.templates.sections.AdvanceDirectiveFactory;
...
public class AdvanceDirectiveSectionTest {
    ...
    public void testBuildingTemplate() throws Exception {
        ...
        for (int n = 0; n < 4; n++) {
            ...
            List<Participant> verifiers = new ArrayList<>();
        ...
        assertEquals("2.16.840.1.113883.10.20.22.2.21.1", advanceDirectivesSection.getTemplateIds().get(0).getRoot());
        ...
        for (AdvanceDirective adMock : advanceDirectives) {
            AdvanceDirectiveObservation observation = (AdvanceDirectiveObservation) TestUtil.getObservationById(adMock.getId(), advanceDirectiveObservations);
...
//            assertEquals(adMock.getId(), observation.getIds().get(0).getRoot());
            ...
            for (int j = 0; j < participants.size(); j++) {
                ...
                if (participant.getTypeCode().getLiteral().equals("VRF")) {
                    ...
                    Telecom telecom = personMock.getTelecoms().get(0);
