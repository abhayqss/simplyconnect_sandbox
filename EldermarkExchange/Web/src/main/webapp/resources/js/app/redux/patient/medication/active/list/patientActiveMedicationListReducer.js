define(
    [
        ...
        path('./PatientActiveMedicationListInitialState')
    ],
    function (varants, InitialState) {
        ...
        return function (state, action) {
            ...
            switch (action.type) {
                case types.CLEAN_PATIENT_ACTIVE_MEDICATIONS:
                    return state
                        .removeIn(['error'])
                        .setIn(['isFetching'], false)
                        .removeIn(['dataSource', 'data']);

                case types.CLEAN_PATIENT_ACTIVE_MEDICATION_LIST_ERROR:
                    return state.removeIn(['error']);

                case types.LOAD_PATIENT_ACTIVE_MEDICATIONS_REQUEST:
                    return state
                        .setIn(['error'], null)
                        .setIn(['isFetching'], true);

                case types.LOAD_PATIENT_ACTIVE_MEDICATIONS_SUCCESS: {
                ...
                case types.LOAD_PATIENT_ACTIVE_MEDICATIONS_FAILURE:
