IF OBJECT_ID('IncidentReportSubmitNotification') IS NOT NULL
DROP TABLE [dbo].[IncidentReportSubmitNotification];
GO

SET ANSI_PADDING OFF
GO

CREATE TABLE IncidentReportSubmitNotification (
  ...
  CONSTRAINT [PK_IncidentReportSubmitNotification] PRIMARY KEY CLUSTERED
(
  [id] ASC
...
ALTER TABLE [dbo].[IncidentReportSubmitNotification] WITH CHECK ADD CONSTRAINT [FK_IncidentReportSubmitNotification_IncidentReport] FOREIGN KEY ([incident_report_id])
REFERENCES [dbo].[IncidentReport] ([id])
GO

ALTER TABLE [dbo].[IncidentReportSubmitNotification] CHECK CONSTRAINT [FK_IncidentReportSubmitNotification_IncidentReport]
GO

ALTER TABLE [dbo].[IncidentReportSubmitNotification] WITH CHECK ADD CONSTRAINT [FK_IncidentReportSubmitNotification_Employee] FOREIGN KEY ([employee_id])
REFERENCES [dbo].[Employee_enc] ([id])
GO

ALTER TABLE [dbo].[IncidentReportSubmitNotification] CHECK CONSTRAINT [FK_IncidentReportSubmitNotification_Employee]
GO

ALTER TABLE [dbo].[Employee_enc]
  ADD [is_incident_report_reviewer] [bit] NOT NULL DEFAULT 0
GO
...
BEGIN
  INSERT INTO Employee_enc
    ([inactive], [legacy_id], [password], [database_id], [person_id], [care_team_role_id], [created_automatically],
     ...
     [creator_id],[labs_coordinator],[is_incident_report_reviewer])
  SELECT
    [inactive],
    [legacy_id],
    [password],
...
BEGIN
  ...
  SET
    ...
    [labs_coordinator]            = i.[labs_coordinator],
    [is_incident_report_reviewer] = i.[is_incident_report_reviewer]
  FROM inserted i
