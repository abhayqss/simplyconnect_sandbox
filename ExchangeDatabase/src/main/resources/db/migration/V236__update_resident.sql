...
WHERE [object_id] = OBJECT_ID('[dbo].[resident_enc]') AND [name] = 'active';
EXEC('ALTER TABLE [dbo].[resident_enc] DROP CONSTRAINT ' + @ObjectName)
GO

ALTER TABLE [dbo].[resident_enc] DROP COLUMN [active]
GO

ALTER TABLE [dbo].[resident_enc] ADD [active] [bit] NOT NULL DEFAULT 1
GO
...
as
 select
	...
	,created_by_id, legacy_table, [active]
...
INSERT INTO resident_enc
	...
	prev_addr_state,prev_addr_zip,advance_directive_free_text,first_name,last_name,middle_name,preferred_name,patient_account_number,birth_place,death_date,mother_account_number, created_by_id, legacy_table, [active])
   SELECT
