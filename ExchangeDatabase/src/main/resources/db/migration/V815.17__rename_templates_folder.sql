alter view CommunityDocumentAndFolderView as
    with community_template_folder as (select org.id as organization_id,
                                              db.id  as database_id
                                       from Organization org
                                                join SourceDatabase db
                                                     on org.database_id = db.id and db.is_signature_enabled = 1),
         community_template as (select community_template_folder.organization_id as organization_id,
                                       dst_sd.signature_template_id              as template_id
                                from DocumentSignatureTemplate_SourceDatabase dst_sd
                                         join community_template_folder
                                              on community_template_folder.database_id = dst_sd.database_id
                                union
                                ...
                                from DocumentSignatureTemplate_Organization dst_o
                                         join community_template_folder
                                              on community_template_folder.organization_id = dst_o.organization_id)
    ...
    select concat('tf', convert(varchar(19), o.id)) as id,
           ...
           'E-sign Documents and Forms'             as title,
           null                                     as folder_id,
           null                                     as mime_type,
           null                                     as size,
           ...
           null                                     as template_id,
           null                                     as template_status
    from community_template_folder
             join Organization o on community_template_folder.organization_id = o.id
go
