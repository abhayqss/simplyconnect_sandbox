if (object_id('NotViewableCareTeam') is not null)
  drop view NotViewableCareTeam
go

create view NotViewableCareTeam as
  with ctmAccess as (
      select
        ctm.employee_id,
        rctm.resident_id,
        IIF(ctmar.access_right_id is null, 0, 1) as can_view
      from CareTeamMember ctm
        join ResidentCareTeamMember rctm on ctm.id = rctm.id
        left join CareTeamMember_AccessRight ctmar
          on rctm.id = ctmar.care_team_member_id and ctmar.access_right_id = (select id
                                                                              from AccessRight
                                                                              where code = 'MY_CT_VISIBILITY')
  ), ctmAndMergedAccess as (
      select
        ctmAccess.employee_id,
        mrv.merged_resident_id as resident_Id,
        ctmAccess.can_view
      from ctmAccess
        join MergedResidentsView mrv on ctmAccess.resident_id = mrv.resident_id
  ), visiblePriority as (
      select
        employee_id,
        resident_id,
        max(can_view) as can_view
      from ctmAndMergedAccess
      group by employee_id, resident_id
  ), notViewableOnly as (
      select
        employee_id,
        resident_id
      from visiblePriority
      where can_view = 0
  )
  select *
  from notViewableOnly
GO
