...
CREATE TRIGGER TRG_MedicationInformation_insert_update
  ...
  BEGIN
    ...
    with upd as (
        ...
        from inserted i
          join dbo.Medication (NOLOCK) m on m.medication_information_id = i.id
    )
...
CREATE TRIGGER TRG_MedicationInformation_delete
  ...
  BEGIN
    ...
    with upd as (
        SELECT distinct resident_id
        from deleted d
          join dbo.Medication (NOLOCK) m on m.medication_information_id = d.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
        upd.resident_id,
...
CREATE TRIGGER TRG_MedicationSupplyOrder_insert_update
  ...
  BEGIN
    ...
    with upd as (
        SELECT distinct resident_id
        from inserted i
          join dbo.Medication (NOLOCK) m on m.medication_supply_order_id = i.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
        upd.resident_id,
...
CREATE TRIGGER TRG_MedicationSupplyOrder_delete
  ...
  BEGIN
    ...
    with upd as (
        SELECT distinct resident_id
        from deleted d
          join dbo.Medication (NOLOCK) m on m.medication_supply_order_id = d.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
        upd.resident_id,
...
CREATE TRIGGER TRG_MedicationDispense_insert_update
  ...
  BEGIN
    ...
    with upd as (
        ...
        from inserted i
          join dbo.Medication_MedicationDispense (NOLOCK) mmd on mmd.medication_dispense_id = i.id
          join dbo.Medication (NOLOCK) m on mmd.medication_dispense_id = m.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
        upd.resident_id,
...
CREATE TRIGGER TRG_MedicationDispense_delete
  ...
  BEGIN
    ...
    with upd as (
        SELECT distinct m.resident_id
        from deleted d
          join dbo.Medication_MedicationDispense (NOLOCK) mmd on mmd.medication_dispense_id = d.id
          join dbo.Medication (NOLOCK) m on mmd.medication_dispense_id = m.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
...
CREATE TRIGGER TRG_ImmunizationMedicationInformation_insert_update
  ...
  BEGIN
    ...
    with upd as (
      SELECT resident_id
      from inserted i
        join dbo.MedicationDispense (NOLOCK) md on md.immunization_medication_information_id = i.id
        join dbo.Medication_MedicationDispense (NOLOCK) mmd on mmd.medication_dispense_id = md.id
        join dbo.Medication (NOLOCK) m on mmd.medication_dispense_id = m.id
      union
      SELECT resident_id
      from inserted i
        join dbo.MedicationSupplyOrder (NOLOCK) mso on mso.immunization_medication_information_id = i.id
        join dbo.Medication (NOLOCK) m on m.medication_supply_order_id = mso.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
        upd.resident_id,
...
CREATE TRIGGER TRG_ImmunizationMedicationInformation_delete
  ...
  BEGIN
    ...
    with upd as (
      SELECT resident_id
      from deleted d
        join dbo.MedicationDispense (NOLOCK) md on md.immunization_medication_information_id = d.id
        join dbo.Medication_MedicationDispense (NOLOCK) mmd on mmd.medication_dispense_id = md.id
        join dbo.Medication (NOLOCK) m on mmd.medication_dispense_id = m.id
      union
      SELECT resident_id
      from deleted d
        join dbo.MedicationSupplyOrder (NOLOCK) mso on mso.immunization_medication_information_id = d.id
        join dbo.Medication (NOLOCK) m on m.medication_supply_order_id = mso.id
    )
    INSERT INTO @arg (resident_id, update_type, update_time)
      SELECT
