...
ALTER TABLE [dbo].[PlanOfCare] ADD [free_text] varchar(max) NULL;
GO

ALTER TABLE [dbo].[PlanOfCare] ADD [document_id] bigint NULL,
  CONSTRAINT FK_PlanOfCare_Document FOREIGN KEY ([document_id]) REFERENCES [dbo].Document ([id])
GO
...
AS
  ...
  -- pagination & output
  SELECT
    RowNum as id, pl_of_care_id, pl_of_care_date, pl_of_care_activity, resident_id, is_free_text
  FROM SortedTable
  WHERE
...
ALTER PROCEDURE [dbo].[load_ccd_plan_of_care_count]
    @ResidentId bigint,
    @Aggregated tinyint
AS
  BEGIN
