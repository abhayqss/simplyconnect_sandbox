IF COL_LENGTH('SourceDatabase', 'is_chat_enabled') IS NOT NULL
  BEGIN
    alter table SourceDatabase
      drop column is_chat_enabled;
  END
GO

alter table SourceDatabase
  add is_chat_enabled bit;
GO

IF COL_LENGTH('Employee_enc', 'twilio_user_sid') IS NOT NULL
  BEGIN
    alter table Employee_enc
      drop column twilio_user_sid;
  END
GO

ALTER TABLE [dbo].[Employee_enc]
  ADD [twilio_user_sid] varchar(34) NULL
GO
...
AS
  BEGIN
    ...
    ([inactive], [legacy_id], [password], [database_id], [person_id], [care_team_role_id], [created_automatically],
     ...
     [creator_id], [labs_coordinator], [is_incident_report_reviewer], [twilio_user_sid])
      SELECT
        [inactive],
        [legacy_id],
        [password],
