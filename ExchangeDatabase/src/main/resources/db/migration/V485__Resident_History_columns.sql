  ...
  AS
    SELECT
      ...
      (hashbytes('SHA1',
                 ...
                 isnull(CONVERT(VARCHAR(11), DecryptByKey(ssn)), 'NA')))             [hash_key], [chain_id],[archived],[status]
    FROM resident_enc;
...
AS
  BEGIN
    ...
    ([legacy_id], [admit_date], [discharge_date], [database_id], [custodian_id], [data_enterer_id], [facility_id], [legal_authenticator_id],
     ...
     [intake_date], [referral_source], [current_pharmacy_name], [consana_xref_id], [chain_id], [archived], [status])
      SELECT
        [legacy_id],
        [admit_date],
        [discharge_date],
        ...
        [consana_xref_id], [chain_id], [archived], [status]
      FROM inserted;
  END;
