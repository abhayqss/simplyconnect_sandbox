if col_length('Employee_enc', 'birth_date') is not null
    begin
...
alter table Employee_enc
    add birth_date varbinary(max) null
go
...
ALTER TRIGGER EmployeeInsert ON Employee
    INSTEAD OF INSERT
    AS
    BEGIN
        INSERT INTO Employee_enc([inactive],
                                 [legacy_id],
                                 [password],
                                 [database_id],
                                 [person_id],
...
ALTER TRIGGER EmployeeUpdate ON Employee
    INSTEAD OF UPDATE
    AS
    BEGIN
        UPDATE Employee_enc
        SET [inactive]                        = i.[inactive],
            ...
            [birth_date]                      = EncryptByKey(Key_GUID('SymmetricKey1'),
                                                             CONVERT(VARCHAR, i.[birth_date], 0))
        FROM inserted i
