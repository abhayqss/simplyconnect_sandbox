ALTER PROCEDURE [dbo].[find_merged_patients]
    @ResidentId BIGINT
AS
  BEGIN
    SET NOCOUNT ON;
    ...
    INSERT INTO @found_residents(resident_id)
      ...
      FROM [dbo].[MPI_merged_residents] mpi
        ...
        INNER JOIN resident r ON r.[id] = mpi.[surviving_resident_id] AND (r.[opt_out] IS NULL OR r.[opt_out] <> 1)
        -- filter out residents from organizations not eligible for discovery by Exchange user
        INNER JOIN Organization o ON o.[id] = r.[facility_id] AND
                                     (o.[testing_training] IS NULL OR o.[testing_training] <> 1) AND
                                     (o.[inactive] IS NULL OR o.[inactive] <> 1)
                                     AND o.[module_hie] = 1 AND o.[legacy_table] = 'Company'
      WHERE mpi.[merged_resident_id] = @ResidentId AND mpi.[merged] = 1
      UNION
      SELECT mpi.[merged_resident_id]
      FROM [dbo].[MPI_merged_residents] mpi
        -- filter out residents not eligible for discovery
        INNER JOIN resident r ON r.[id] = mpi.[merged_resident_id] AND (r.[opt_out] IS NULL OR r.[opt_out] <> 1)
        -- filter out residents from organizations not eligible for discovery by Exchange user
        INNER JOIN Organization o ON o.[id] = r.[facility_id] AND
                                     (o.[testing_training] is null or o.[testing_training] <> 1) AND
                                     (o.[inactive] IS NULL OR o.[inactive] <> 1)
