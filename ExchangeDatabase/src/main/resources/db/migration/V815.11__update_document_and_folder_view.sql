alter view CommunityDocumentAndFolderView as
    with community_template_folder as (select org.id as orgnanization_id,
                                              db.id  as database_id
                                       from Organization org
                                                join SourceDatabase db
                                                     on org.database_id = db.id and db.is_signature_enabled = 1),
         community_template as (select community_template_folder.orgnanization_id as organization_id,
                                ...
                                from DocumentSignatureTemplate_Organization dst_o
                                         join community_template_folder
                                              on community_template_folder.database_id = dst_o.organization_id)
    ...
    from community_template_folder
             join Organization o on community_template_folder.orgnanization_id = o.id
go
