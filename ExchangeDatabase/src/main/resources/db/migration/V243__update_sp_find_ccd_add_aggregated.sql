...
BEGIN
	...
	-- pagination & output
	SELECT 
		RowNum as id, ad_type as 'directive_type', ad_verifiers as 'directive_verification', ad_ref_docs as 'directive_supporting_documents', 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT o.id, o.product_text, o_status_code.display_name, r.reaction_text FROM Allergy a
	...
	-- pagination & output
	SELECT 
		RowNum as id, observation_product_text, observation_status_code_text, observation_reactions, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT e.id, e.encounter_type_text, cc.display_name, e.effective_time  
		FROM Encounter e
	...
	-- pagination & output
	SELECT 
		RowNum as id, encounter_type_text, encounter_date, encounter_provider_codes, encounter_service_delivery_locations, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
...
BEGIN
	...
	-- pagination & output
	SELECT 
		RowNum as id, fh_problem_name, fh_age_observation_val, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum	
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT i.id, imi.text, i.immunization_stopped, i.immunization_started,  i.status_code FROM Immunization i 
	...
	-- pagination & output
	SELECT 
		RowNum as id, imm_text, imm_started, imm_stopped, imm_status, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT me.id, cc.display_name, me.effective_time_high
	...
	-- pagination & output
	SELECT 
		RowNum as id, medical_equipment_device, medical_equipment_time_heigh, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
...
BEGIN
	...
	-- pagination & output
	SELECT 
		RowNum as id, mdc_stopped, mdc_started, mdc_free_text_sig, mdc_status_code, mdc_info_product_name_text, instr_text, indications, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT pr.id, o.name, pa.participant_member_id, prt.effective_time_high, prt.effective_time_low
	FROM Payer pr
	...
	-- pagination & output
	SELECT 
		RowNum as id, payer_providers_insurance_info, payer_providers_insurance_member_id, payer_providers_time_low, payer_providers_time_heigh, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
	SELECT DISTINCT poc.id, cc.display_name, poc_act.effective_time
	FROM  PlanOfCare poc
	...
	-- pagination & output
	SELECT 
		RowNum as id, pl_of_care_date, pl_of_care_activity, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
...
BEGIN
	...
	(
		SELECT 
			o.problem_name, o.effective_time_low, o.effective_time_high, o.problem_status_text,o.problem_value_code_icd9,
			ROW_NUMBER() OVER (
				ORDER BY
					...
					CASE WHEN @SortBy = 'problem_value_code_icd9' AND @SortDir = 'ASC' THEN o.problem_value_code_icd9 END ASC,
					CASE WHEN @SortBy = 'problem_value_code_icd9' AND @SortDir = 'DESC' THEN o.problem_value_code_icd9 END DESC
			) AS RowNum
		FROM Problem p
	...
	-- pagination & output
	SELECT
		RowNum as id, problem_name, effective_time_low, effective_time_high, problem_status_text,problem_value_code_icd9, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit)
	ORDER BY RowNum
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT a.procedure_type_text, a.procedure_started, a.procedure_stopped
	...
	-- pagination & output
	SELECT 
		RowNum as id, procedure_type_text, procedure_started, procedure_stopped, 'Simply Connect' as 'data_source'  
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 
...
BEGIN
	...
	-- select data
	INSERT INTO @T 
		SELECT DISTINCT ro.id, ro.effective_time, ro.result_text, ro.status_code, cc.display_name, ro.result_value, ro.result_value_unit 
		FROM Result rs left JOIN
              ...
              CcdCode cc ON roic.interpretation_code_id = cc.id
		WHERE resident_id in (select resident_id from @found_residents)
	...
	(
		SELECT
			...
			STUFF (
				(SELECT ', ' + p.result_ref_range FROM @S as p WHERE p.result_id = t.result_id FOR XML PATH('')), 1, 1, ''
		...
		GROUP BY	
			t.result_id,
			t.result_date, t.result_text, t.result_status_code, t.result_value, t.result_value_unit
	)
	
	-- pagination & output
	SELECT 
		RowNum as id, result_date, result_text, result_status_code, result_val_unit, result_interpretation_codes, result_ref_ranges, 'Simply Connect' as 'data_source'
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
...
BEGIN
	...
	-- select data
	INSERT INTO @T
		SELECT DISTINCT sho.free_text, cc.display_name
	...
	-- pagination & output
	SELECT
		RowNum as id, s_history_free_text, s_history_observation_value, 'Simply Connect' as 'data_source'
	FROM SortedTable
	WHERE
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit)
	ORDER BY RowNum
...
BEGIN
	...
	-- pagination & output
	SELECT 
		RowNum as id, vs_date, vs_value, vs_res_type, 'Simply Connect' as 'data_source' 
	FROM SortedTable
	WHERE 
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit) 
	ORDER BY RowNum 	
