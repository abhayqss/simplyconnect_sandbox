...
CREATE view [dbo].[ClientPharmacyFilterView] as with
    comprehensives as (
        select id,
               resident_id,
    ...
    active_or_unknown_medications as (
        ...
        from medication
        where (status_code != 'completed') OR (status_code is null and (medication_stopped is null OR medication_stopped > GETDATE()))
	...
	SELECT r.id as resident_id, ISNULL(p.pharmacy_name, '#NO_PHARMACY_FOUND_FOR_CLIENT') as pharmacy_name, r.facility_id, r.database_id FROM [dbo].[resident_enc] r left join result_clients p on  p.resident_id = r.id
