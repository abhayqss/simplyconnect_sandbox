IF COL_LENGTH('SourceDatabase', 'is_video_enabled') IS NOT NULL
  BEGIN
    alter table SourceDatabase
      drop column is_video_enabled;
  END
GO

alter table SourceDatabase
  add is_video_enabled bit;
GO

IF COL_LENGTH('Employee_enc', 'twilio_service_conversation_sid') IS NOT NULL
  BEGIN
    alter table Employee_enc
      drop column twilio_service_conversation_sid;
  END
GO

ALTER TABLE [dbo].[Employee_enc]
  ADD [twilio_service_conversation_sid] varchar(40) NULL
GO
