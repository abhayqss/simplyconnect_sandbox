...
IF (OBJECT_ID('[dbo].[find_probably_matched_patients]') IS NOT NULL)
  DROP PROCEDURE [dbo].[find_probably_matched_patients]
GO


CREATE PROCEDURE [dbo].[find_probably_matched_patients]
	@ResidentId bigint
AS
BEGIN
	SET NOCOUNT ON;
	...
	declare @current_step_found_matched_residents table (
		resident_id bigint
	)
	...
	begin
		...
		delete from @current_step_found_matched_residents;
		...
		insert into @current_step_found_surviving_residents(resident_id) 
			(select surviving_resident_id from dbo.MPI_merged_residents where merged_resident_id in (select resident_id from @current_step_residents_to_search) 
																	and surviving_resident_id not in (select resident_id from @found_residents) and probably_matched = 1 and merged <> 1);

		insert into @current_step_found_matched_residents(resident_id)
			(select merged_resident_id from dbo.MPI_merged_residents where surviving_resident_id in (select resident_id from @current_step_residents_to_search)
																	and merged_resident_id not in (select resident_id from @found_residents) and probably_matched = 1 and merged <> 1);
		...
		insert into @found_residents(resident_id)
			(select resident_id from @current_step_found_matched_residents r1 where not exists(select * from @found_residents r2 where r2.resident_id = r1.resident_id));
		...
		insert into @current_step_residents_to_search(resident_id)
			(select resident_id from @current_step_found_matched_residents r1 where not exists(select * from @current_step_residents_to_search r2 where r2.resident_id = r1.resident_id));
