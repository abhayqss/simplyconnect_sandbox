IF (OBJECT_ID('DeactivateEmployeeNotification') IS NOT NULL)
    DROP TABLE [dbo].[DeactivateEmployeeNotification]
GO

CREATE TABLE [dbo].[DeactivateEmployeeNotification]
(
    ...
    CONSTRAINT [PK_DeactivateEmployeeNotification] PRIMARY KEY CLUSTERED
        ([id] ASC) WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
GO

ALTER TABLE [dbo].[DeactivateEmployeeNotification]
    WITH CHECK ADD CONSTRAINT [FK_DeactivateEmployeeNotification_Employee] FOREIGN KEY ([employee_id])
        REFERENCES [dbo].[Employee_enc] ([id])
GO

ALTER TABLE [dbo].[DeactivateEmployeeNotification]
    CHECK CONSTRAINT [FK_DeactivateEmployeeNotification_Employee]
GO

ALTER TABLE [dbo].[Employee_enc]
    ADD [is_auto_status_changed] [bit] NOT NULL
        CONSTRAINT DF_Employee_enc_auto_status_changed DEFAULT (0)
GO

ALTER TABLE [dbo].[Employee_enc]
    ADD [deactivate_datetime] [datetime2](7) NULL
GO

UPDATE Employee_enc
SET [inactive]               = 3,
    [is_auto_status_changed] = 1,
    [deactivate_datetime]    = CURRENT_TIMESTAMP
WHERE Employee_enc.id IN (SELECT EM.id
                          FROM Employee_enc EM,
                               (SELECT MAX(date) as maxDate, employee_id FROM AuditLog WHERE action = 'LOG_IN' GROUP BY employee_id) as AL
                          WHERE EM.id = AL.employee_id
                            AND EM.inactive = 0
                            AND DATEDIFF(day, maxDate, GETDATE()) >= 90)
GO
...
BEGIN
    ...
    ([inactive], [legacy_id], [password], [database_id], [person_id], [care_team_role_id], [created_automatically],
     ...
     [is_community_address_used], [is_auto_status_changed], [deactivate_datetime])
    SELECT [inactive],
           [legacy_id],
           [password],
           [database_id],
