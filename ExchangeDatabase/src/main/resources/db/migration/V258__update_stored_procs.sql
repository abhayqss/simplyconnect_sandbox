...
BEGIN
	...
	-- select data
	INSERT INTO @T
		SELECT DISTINCT i.id, imi.text, i.immunization_stopped, i.immunization_started,  i.status_code, i.resident_id FROM Immunization i
...
BEGIN
	...
	-- pagination & output
	SELECT
		RowNum as id, medical_equipment_device, medical_equipment_time_heigh, resident_id
	FROM SortedTable
	WHERE
		(RowNum > @Offset) AND ((@Limit IS NULL) OR RowNum <= @Offset + @Limit)
	ORDER BY RowNum
...
BEGIN
	...
	(
		SELECT
			o.problem_name, o.effective_time_low, o.effective_time_high, o.problem_status_text, o.problem_value_code, o.problem_value_code_set, p.resident_id,
			ROW_NUMBER() OVER (
				ORDER BY
