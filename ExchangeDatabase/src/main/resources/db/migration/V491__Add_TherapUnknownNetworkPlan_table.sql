CREATE TABLE [dbo].[TherapUnknownNetworkPlan] (
  [id]              [bigint] IDENTITY (1, 1) NOT NULL,
  [med_plan_id]     [varchar](200)           NULL,
  [med_plan_name]   [varchar](200)           NULL,
  [med_plan_issuer] [varchar](255)           NULL
)
GO
...
ALTER TRIGGER [dbo].[ResidentInsert]
ON [dbo].[resident]
INSTEAD OF INSERT
AS
BEGIN
  ...
  ([legacy_id], [admit_date], [discharge_date], [database_id], [custodian_id], [data_enterer_id], [facility_id], [legal_authenticator_id],
   ...
   [dental_insurance], [citizenship], [birth_order], [death_indicator], [mother_person_id], [status], [ssn], [birth_date], [medical_record_number],
...
ALTER TRIGGER [dbo].[ResidentUpdate]
ON [dbo].[resident]
INSTEAD OF UPDATE
AS
BEGIN
