alter view DocumentSignatureHistoryView as
    with history as (select d.id                   document_id,
    ...
    select h.document_id,
           h.request_id,
           iif(exists(select 1
                      from DocumentSignatureRequestSubmittedField_enc f
                      where f.signature_request_id = r.id
                        and f.pdcflow_overlay_type in (1, 4)),
    ...
    select h.document_id,
           h.request_id,
           'DOCUMENT_RECEIVED'                                          action,
           'Document received'                                          action_title,
           iif(e.id is null, c.first_name, e.first_name)                actor_first_name,
           iif(e.id is null, c.last_name, e.last_name)                  actor_last_name,
           iif(e.id is null, roles.client_role_id, e.care_team_role_id) actor_role_id,
           e.id                                                         employee_id,
