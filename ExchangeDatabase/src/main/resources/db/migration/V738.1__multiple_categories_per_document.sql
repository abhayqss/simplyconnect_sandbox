create table Document_DocumentCategory
(
    document_id       bigint not null,
    category_chain_id bigint not null,
    constraint PK_Document_DocumentCategory primary key (document_id, category_chain_id),
    constraint FK_Document foreign key (document_id) references Document (id)
)
go

create view Document_DocumentCategoryView
as
...
insert into Document_DocumentCategory(document_id, category_chain_id)
select d.id as document_id, d.category_chain_id as category_chain_id
from Document d
where d.category_chain_id is not null
go

alter table Document
    drop column category_chain_id
go
