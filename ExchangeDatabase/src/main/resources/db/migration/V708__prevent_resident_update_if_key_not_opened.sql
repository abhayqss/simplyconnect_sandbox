if object_id('ResidentCorruptActionLog') is not null
    drop table ResidentCorruptActionLog
go

create table ResidentCorruptActionLog
(
    id            bigint identity
        constraint PK_ReidentCorruptActionLog PRIMARY KEY,
    resident_id   bigint,
...
if object_id('sp_track_resident_corruption') is not null
    drop table ResidentCorruptActionLog
go

CREATE PROCEDURE sp_track_resident_corruption @ResidentId bigint,
                                              @Action varchar(6)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    insert into ResidentCorruptActionLog (resident_id, action_date, action_type, SPID, Login, HostName, DBName, Command,
                                          ProgramName, SPID2)
    select @ResidentId,
           getdate(),
           @Action,
