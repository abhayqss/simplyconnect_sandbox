...
public class JdbcAuditUserDetailsService extends JdbcDaoSupport implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String input) throws UsernameNotFoundException, DataAccessException
    {
        ...
        authorities.addAll(loadSystemRole(user.getId()));
    ...
    private List<GrantedAuthority> loadSystemRole(Long employeeId) {
        String sql =
                "SELECT [CareTeamRole].[code] " +
                        "   FROM [dbo].[Employee_enc] " +
                        "       JOIN [dbo].[CareTeamRole] ON [Employee_enc].[care_team_role_id] = [CareTeamRole].id " +
                        "   WHERE [Employee_enc].[id] = ?";

        return getJdbcTemplate().query(sql, new Object[] {employeeId}, new RowMapper<GrantedAuthority>() {
            @Override
            public GrantedAuthority mapRow(ResultSet rs, int rowNum) throws SQLException {
                String roleName = rs.getString("code");
                return new SimpleGrantedAuthority(roleName);
            }
        });
    }
