...
public class SubmitObjectsRequest extends XdsCommon {
	...
	public OMElement submitObjectsRequest(OMElement sor) {
		...
		try {
			sor.build();
			if ("DeprecateObjectsRequest".equals(sor.getLocalName())) {
				deprecateObjectRequestInternal(sor);
			} else if ("ApproveObjectsRequest".equals(sor.getLocalName())) {
				approveObjectRequestInternal(sor);
			} else {
	...
	XdsException, XdsInternalException {
		if (Properties.loader().getBoolean("validate.patient.id")) {
			try {
				...
				PatientIdentifier pid = XdsEntityUtil.convertPatientIdentifier(patient_id, connection);
	...
	private boolean deprecateObjectRequestInternal(OMElement sor) {
		boolean status = true;
		try {
			XdsRegistryLifeCycleService lcm = XdsFactory.getXdsRegistryLifeCycleService();
			lcm.deprecateObjects(sor, new RegistryLifeCycleContext());
		} catch(RegistryLifeCycleException e) {
			response.add_error("XDSRegistryError", e.getMessage(), RegistryUtility.exception_details(e),log_message);
			status = false;
		}
	...
	private boolean approveObjectRequestInternal(OMElement sor) {
		boolean status = true;
		try {
			XdsRegistryLifeCycleService lcm = XdsFactory.getXdsRegistryLifeCycleService();
			lcm.approveObjects(sor, new RegistryLifeCycleContext());
		} catch(RegistryLifeCycleException e) {
			response.add_error("XDSRegistryError", e.getMessage(), RegistryUtility.exception_details(e),log_message);
			status = false;
		}
