...
import gov.nist.registry.common2.registry.*;
...
import org.openhealthtools.openxds.repository.api.*;
...
public class ProvideAndRegisterDocumentSet extends XdsCommon {
	...
	private final LssiPv1PatientIdService lssiPv1PatientIdService;
    private static final String PATIENT_IDS_XPATH = "//*[@" + MetadataSupport.identificationscheme_qname + "='" + MetadataSupport.XDSDocumentEntry_patientid_uuid + "' or " +
            "@" + MetadataSupport.identificationscheme_qname + "='" + MetadataSupport.XDSSubmissionSet_patientid_uuid + "']";
...
//	}
	...
	public ProvideAndRegisterDocumentSet(LogMessage log_message, short xds_version, MessageContext messageContext) {
        ...
        this.lssiPv1PatientIdService = XdsFactory.getInstance().getBean(LssiPv1PatientIdService.class);
    ...
    public void updateNamespaceForLssi(OMElement sor) {
        try {
            final XPath patientXpath = new AXIOMXPath(PATIENT_IDS_XPATH);
            final SimpleNamespaceContext nsCtx = new SimpleNamespaceContext();
            patientXpath.setNamespaceContext(nsCtx);

            final List<OMElement> patientIdNodes = patientXpath.selectNodes(sor);

            for (OMElement patientIdNode : patientIdNodes) {
                final OMAttribute omAttribute = patientIdNode.getAttribute(MetadataSupport.value_qname);
                omAttribute.setAttributeValue(lssiPv1PatientIdService.findExistingIdentifier(omAttribute.getAttributeValue()));
            }

        } catch (JaxenException e) {
            logger.info("Couldn't find patient id nodes in OMElement " + sor.toString(), e);
        }
    }
...
//		val.run();
	...
	//todo move out common parts with store_document_mtom
    ...
    throws MetadataException, XdsIOException, XdsInternalException, XdsConfigurationException, XdsException, XDSRepositoryMetadataException {
		...
		String documentName = m.getNameValue(id);
		item.setDocumentTitle(documentName);

		String documentUuid = extrinsic_object.getAttributeValue(MetadataSupport.id_qname);
		item.setDocumentUUID(documentUuid);

		final String patientId = m.getSubmissionSetPatientId();
		item.setPatientId(XdsEntityUtil.convertPatientIdentifier(patientId, connection));
	...
	throws MetadataException, XdsIOException, XdsInternalException, XdsConfigurationException, XdsException, XDSRepositoryMetadataException {
        ...
        DataHandler dataHandler = new DataHandler(new ByteArrayDataSource(bytes, mime_type));

				/* new DataHandler(new String(bytes), mime_type);*/
