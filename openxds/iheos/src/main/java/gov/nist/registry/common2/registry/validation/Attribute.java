...
public class Attribute {
	...
	String validate_CX_datatype(String pid) {
		...
		String[] aaAndAf = part2.split("\\^\\^");

		String[] partsa = aaAndAf[0].split("&");
		if (partsa.length != 3)
		...
		if (aaAndAf.length == 2) {
			String[] partAF = aaAndAf[1].split("&");
			if (partAF.length != 3) {
				return "Invalid assigning facility lentgh";
			}
			if (partAF[1].length() == 0) {
				return "Expected assigning facility universal id";
			}
		}
