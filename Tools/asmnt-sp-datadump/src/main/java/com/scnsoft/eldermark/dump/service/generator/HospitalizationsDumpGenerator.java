...
public class HospitalizationsDumpGenerator implements DumpGenerator {
    ...
    public List<Dump> generateDump(DumpFilter filter) {
        var report = new HospitalizationsDump();
    ...
    private List<HospitalizationEventRow> eventRows(DumpFilter filter) {
        var byHospitalizationType = eventSpecificationGenerator.byEventType(EventTypeEnum.H);
        var betweenDates = eventSpecificationGenerator.betweenDates(filter.getFromAtDefaultZone(), filter.getToAtDefaultZone());
        return eventDao.findAll(eventSpecificationGenerator.byClientOrganizationId(filter.getOrganizationId()).and(betweenDates).and(byHospitalizationType))
                .stream()
                .map(event -> {
    ...
    private List<HospitalizationEventRow> caRows(DumpFilter filter) {
        ...
        return assessmentResultDao.findAll(ca.and(within).and(latest).and(inOrg)).stream()
                .flatMap(assessment -> {
                    ...
                    var lastHospitalizationOpt = parseLastHospitalAdmissionDate(parsed.getLastHospitalAdmissionDate());
    ...
    private Optional<Instant> parseLastHospitalAdmissionDate(String s) {
