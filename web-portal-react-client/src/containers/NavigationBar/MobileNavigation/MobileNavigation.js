...
const NavItems = memo(function NavItems(
    { items, onClickItem, isItemActive }
) {
    return (
        <>
            {items.map(o => (
                <div
                    ...
                    className={cn(
                        ...
                        { 'NavigationMobile-Item__Active': isItemActive(o) }
                    )}
                >
...
function MobileNavigation({ items }) {
    ...
    const isNavItemActive = useCallback(
        o => location.pathname.includes(getHref(o)),
        [location.pathname]
    )
    ...
    return (
        <OutsideClickListener
            onClick={close}
            className="NavigationMobile hide-on-tablet-and-laptop"
        >
            <div className="NavigationMobile-Bar">
                ...
                <div>
                    {loggedIn && (
                        <User
                            avatarSize={40}
                            className="NavigationMobile-User"
                        />
                    )}
                </div>
            </div>
            ...
            {loggedIn && (
                <div
                    ref={listRef}
                    className={cn(
                        'NavigationMobile-Navs',
                        { 'NavigationMobile-Navs_open': isOpen }
                    )}
                >
                    <div className="NavigationMobile-Navs-Buttons">
                        <Cross
                            onClick={close}
                            className="NavigationMobile-CrossIcon"
                        />
                        <MobileLogo
                            height={44}
                            className="NavigationMobile-Logo"
                        />
                    </div>
