...
import { useChatBadge } from "containers/SDKChat/context/ChatBadgeContext";
...
function NavItems({ items, getItemLinkStyle }) {
  return (
    <>
      {items.map((o) => (
        <NavItem key={o.href} className="Navigation-Item" style={{ position: "relative" }}>
...
function NavigationBar({
  ...
  const { unreadChatsCount } = useChatBadge();
  ...
  const isROLE_COMMUNITY_DOCTOR = user?.roleName === ROLE_COMMUNITY_DOCTOR;
  ...
  const permissions = useMemo(
    () => ({
      ...
      REFERRALS:
        isROLE_COMMUNITY_DOCTOR ||
        (!isExtProvider && isProfessionalRole && ![PHARMACIST, PHARMACY_TECHNICIAN].includes(user?.roleName)),
      NOTIFY: false,
      EVENTS: ![
        SUPER_ADMINISTRATOR,
        ORGANIZATION_ADMIN,
      ...
      ADMIN:
        (isProfessionalRole || isContentCreatorRole || isVendorConcierge || isROLE_COMMUNITY_DOCTOR) && !isExtProvider,
  ...
  const behaviorReducer = useCallback(
    (item) => {
      switch (item.name) {
        case "CHATS":
          // 注释旧逻辑: hasIndicator: hasNewMessages,
          // 改用unreadChatsCount数字徽章
          return {
            ...item,
            hasIndicator: unreadChatsCount > 0,
            unreadChatsCount,
          };
    ...
    [history, clientRoute, prospectRoute, unreadChatsCount],
  );
