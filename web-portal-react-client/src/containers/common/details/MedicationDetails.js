...
import { ErrorViewer, DataLoadable } from "components";
...
function MedicationDetails({ clientId, medicationId }) {
  ...
  const { data, error, isFetching } = useMedicationQuery(
  ...
  const onCloseErrorViewer = useCallback(() => toggleErrorViewer(false), []);

  return (
    <>
      <DataLoadable data={data} isLoading={isFetching}>
        {(data) => <Content data={data} />}
      </DataLoadable>

      {isErrorViewerOpen && !isIgnoredError(error) && <ErrorViewer isOpen error={error} onClose={onCloseErrorViewer} />}
    </>
  );
}
