...
function MarketplaceFormFields({
  ...
  function addReferralEmail() {
    onChangeField("marketplace.referralEmails", fields.marketplace.referralEmails.push(ReferralEmail()), true);
  }

  function removeReferralEmail(value) {
    let emails = fields.marketplace.referralEmails.filter((email) => email !== value);

    if (emails.size === 0) {
      emails = emails.push(ReferralEmail());
    }

    onChangeField("marketplace.referralEmails", emails, true);
  }
  ...
  const onChangeCategoryField = useCallback(
    (name, value) => {
      ...
      onChangeField("marketplace.serviceIds", [], true);
    },
    [onChangeField],
  );

  const onChangeCheckboxField = (name, value) => {
    onChangeField(name, value, true);
      ...
      <div className="Marketplace">
          ...
          <Row>
            <Col lg={6}>
              <CheckboxField
                ...
                renderLabelIcon={() => (
                  <Info id="ConfirmVisibilityHint" className="CommunityForm-InfoHint ConfirmVisibilityHint" />
                )}
                tooltip={{
                  placement: "right",
            ...
            <Col lg={6}>
              <CheckboxField
                ...
                renderLabelIcon={() => (
                  <Info id="AllowInboundRRHint" className="CommunityForm-InfoHint AllowInboundRRHint" />
                )}
                tooltip={{
                  placement: "right",
        ...
        <div className="CommunityForm-Section Marketplace-Section">
          <div className="CommunityForm-SectionTitle CommunityForm-SectionTitle_withButton">
            Referrals
            <Button color="success" onClick={addReferralEmail} disabled={!permissions.canEditMarketplaceReferralEmails}>
              Add Email
            </Button>
          </div>
          ...
          <Row>
            {fields.marketplace.referralEmails?.map((email, index) => {
              ...
              let canBeRemoved = (!email.canEdit || !isFirst) && permissions.canEditMarketplaceReferralEmails;
              ...
              return (
                <Col md={6} key={index}>
                  <TextField
                    ...
                    isDisabled={!(email.canEdit && permissions.canEditMarketplaceReferralEmails)}
        ...
        <div className="CommunityForm-Section Marketplace-Section">
          <FeaturedServiceProviderSection
            ...
            onChange={(data) => onChangeFormField("marketplace.featuredCommunities", data)}
          />
        </div>
      </div>
