...
import { findWhere, groupBy, isNumber, map, sortBy } from "underscore";
...
import { useSelectOptions, useToggle } from "hooks/common";

import { useClientsQuery, useStatesQuery } from "hooks/business/directory/query";
...
const HIE_CONSENT_POLICY_OPTIONS = [
  { value: HIE_CONSENT_POLICIES.OPT_IN, label: "Yes" },
  { value: HIE_CONSENT_POLICIES.OPT_OUT, label: "No" },
];
...
function SettingsFormFields({
  ...
  const onConfirmToggleSecurityPinDialog = useCallback(() => {
    onChangeField("signatureConfig.isPinEnabled", !fields.signatureConfig.isPinEnabled, true);
