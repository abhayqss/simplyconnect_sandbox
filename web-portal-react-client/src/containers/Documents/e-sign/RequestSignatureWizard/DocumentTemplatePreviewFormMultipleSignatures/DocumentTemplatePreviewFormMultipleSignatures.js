...
import DocumentSignatureSelect from "../DocumentSignatureSelect/DocumentSignatureSelect";
...
function DocumentTemplatePreviewFormMultipleSignatures(
    {
        clientId,
        documentId,
        communityIds,
    ...
    const [isSelectedModalShow, setIsSelectedModalShow] = useState(false)
    ...
    const [selectedTextObjects,setSelectedTextObjects]= useState()
    ...
    const turnBoxIntoPlaceholder = useCallback((target, canvas) => {
        target.set({
         ...
         // 清除text 重新渲染
         canvas.remove(selectedTextObjects[target.payload.id]);
         removeTextObject(target.payload.id)
         removeSignatureArea(target.payload)
         canvas.requestRenderAll(); // 重新渲染 canvas
        setIsConfirmed(false);
    }, [removeSignatureArea])

    const turnBoxIntoSignatureArea = useCallback((target, canvas) => {
        setIsSelectedModalShow(true)
        target.set({
            fill: '#fff1ca',
            borderColor: '#0064ad',
            stroke: undefined,
    ...
    const onClickBox = useCallback(e => {
        if (e.target) {
            ...
            setTargetTop(e.target.top)
            setTargetLeft(e.target.left)
            setSignId(e.target.payload.id)
    ...
    const  closeModal = () =>{
        ...
        removeSignatureArea(clickTarget.payload)
        canvas.requestRenderAll()
        setIsSelectedModalShow(false)
    }

    const onConfirmSelectModal = () =>{
        setIsConfirmed(true);
        setIsSelectedModalShow(false)
    }
// 存储文本对象
    const addTextObject = (signId, textObject) => {
        setSelectedTextObjects(prevTextObjects => ({
            ...prevTextObjects,
            [signId]: textObject,
        }));
    };
...
// 删除文本对象
    const removeTextObject = (signId) => {
        setSelectedTextObjects(prevTextObjects => {
            const updatedTextObjects = { ...prevTextObjects };
            delete updatedTextObjects[signId];
            return updatedTextObjects;
        });
    };


    const OptionsList =  JSON.parse(sessionStorage.getItem('selectedClientsOptions') || '[]')
    useEffect(() => {
        if (isConfirmed) {
            const selectedPerson = OptionsList.map(item =>{
                if(item.value === selectData) return item.fullName
            })
            const { canvas } = clickTarget

            const text = new fabric.Text(`${selectedPerson}`, {
                fontSize: 12,
                fontFamily: 'Arial',
                top: targetTop + 10,
                left: targetLeft + 10,
            ...
            canvas.add(text);
            addTextObject(signId, text)
            addSignatureArea(clickTarget.payload)
            canvas.requestRenderAll()
            setIsConfirmed(false);
        }
    }, [isConfirmed]);
    ...
    return (
        <>
            {
                isSelectedModalShow && signId && <DocumentSignatureSelect
                    isOpen={isSelectedModalShow}
                    onClose={closeModal}
                    changeSelect={changeSelect}
                    selectItem={selectData}
                    signId={signId}
                    options={OptionsList}
                    onConfirm={onConfirmSelectModal}
                />
            }
...
export default memo(DocumentTemplatePreviewFormMultipleSignatures)
