...
function InquiryForm(
	...
	} = useServicesQuery({ isAuthorizedAccess: false })
	...
	const serviceSections = useMemo(() => {
		return sortBy(map(
			groupBy(filteredServices, 'serviceCategoryId'),
			(data, id) => ({
				id: +id,
			...
			<Form className="MarketplaceInquiryForm" onSubmit={onSubmit}>
				...
				<Scrollable style={scrollableStyles} className="MarketplaceInquiryForm-Sections">
					<div className="MarketplaceInquiryForm-Section">
						<div className="MarketplaceInquiryForm-SectionTitle">
							Inquiry
						</div>
						<Row>
							<Col md={6}>
							...
							<Col md={6}>
								<PhoneField
									...
									renderLabelIcon={() => (
										<Info
											id="phone-hint"
											className="MarketplaceInquiryForm-LabelIcon"
										/>
									)}
									tooltip={{
										target: 'phone-hint',
