...
const ClientCareTeamSection = ({ vendorId, errors, onChangeField }) => {
  ...
  const onSelectTeamMember = (data) => {
    ...
    if (index === -1) {
      ...
      if (CLINICAL_ROLE_CODE.includes(data?.careTeamRoleCode)) {
        changeTeamType(data.id, "clinical").then(() => {
          ...
          let newArray = vendorTeamData.map((item) => {
            if (item.id === data.id) {
              return {
                ...item,
                checked: item.careTeamRoleCode === "ROLE_VENDOR_CODE",
                teamType: "clinical",
        ...
        let newArray = vendorTeamData.map((item) => {
          if (item.id === data.id) {
            return { ...item, checked: true, canChangeSelected: true };
          } else return item;
        });
        setVendorTeamData(newArray);
      }
