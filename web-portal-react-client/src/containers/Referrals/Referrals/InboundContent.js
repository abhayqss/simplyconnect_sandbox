...
import { setInboundData, setPagination, setSort } from "redux/ReferralData/tableDataActions";
...
const InboundContent = ({ friendlyName, type, isClient, STATUS_COLORS, DATE_FORMAT }) => {
  const dispatch = useDispatch();

  const isExternalProvider = useExternalProviderRoleCheck();
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  const { inboundData, pagination, sorting } = useSelector((state) => state.ReferData);
  ...
  const fetchData = (params) => {
    ...
    service.find(params).then((res) => {
      dispatch(setInboundData(res.data));
  ...
  return (
    <>
      ...
      <Collapse isOpen={isFilterOpen}>
        <ReferralFilter type={INBOUND} className="margin-bottom-50" onApply={onApply} />
      </Collapse>
