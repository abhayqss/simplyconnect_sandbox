...
const MobileReferralsReply = () => {
  ...
  const [logoTextInfo, setLogoTextInfo] = useState("");
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.data) {
        setLogoText("Thank you for your reply");
        setLogoTextInfo("Your response has been logged in the Simply Connect platform.");
      } else {
        setLogoText("Reply Already Sent");
        setLogoTextInfo("You have already responded to this message.");
      }
    ...
    <div className={"mobileReferralsReply"}>
      ...
      {!isLoading && (
        <div className="mobileReferralsReplyBox">
          ...
          >
            <span>Yes</span>
            {data.replyTime && data.status === 1 && <span>({moment(data.replyTime).format("DD/MM hh:mm A")})</span>}
          </div>
          ...
          >
            <span>No</span>
            {data.replyTime && data.status === 2 && <span>({moment(data.replyTime).format("DD/MM hh:mm A")})</span>}
          </div>
        </div>
      )}
      ...
      {dialogOpen && (
        <div className="modal">
          <div className="modalContent">
            ...
            <div className="contentTitle">{logoText}</div>

            <div className="contentText">{logoTextInfo}</div>
          </div>
        </div>
      )}
    </div>
