...
const WorkflowLeft = (props) => {
  ...
  useEffect(() => {
    ...
    // 如果使用防抖
    const debouncedGetWorkflowList = debounce(() => {
      getWorkflowList(params, signal, requestId);
    }, 300); // 调整时间间隔

    // 调用防抖请求
    debouncedGetWorkflowList();

    return () => {
      controller.abort();
      // 清除防抖计时器
      debouncedGetWorkflowList.cancel();
    };
  ...
  const getWorkflowList = (params, signal, requestId) => {
    // 将请求标识符传入
    setIsFetching(true);
    workflowService
      .findWorkflowForAdmin(params, { signal })
      .then((res) => {
        if (res.success) {
          // 检查请求标识符
