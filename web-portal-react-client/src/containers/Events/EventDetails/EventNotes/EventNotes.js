...
class EventNotes extends Component {
  ...
  update(isReload, page) {
    ...
    if (isReload || shouldReload || (!isFetching && isEmpty(ds.data))) {
      const { eventId, clientId, organizationId } = this.props;
  ...
  render() {
    const { error, clientId, isFetching, organizationId, dataSource: ds } = this.props;

    const { selected, isViewerOpen } = this.state;
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              text: "Status",
              formatter: (v, row) => (
                <span className="EventNote-Status" style={{ backgroundColor: STATUS_COLORS[v] }}>
                  {row.statusTitle}
                </span>
              ),
            },
            ...
            },
            {
              ...
              formatter: (v, row) => (
                <Button id={row.id} color="link" className="EventNote-ViewBtn" onClick={this.onOpenViewer}>
                  View Details
                </Button>
              ),
            },
