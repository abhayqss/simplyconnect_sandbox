...
import { useCommunityCareTeamMembersQuery } from "hooks/business/conversations";

import BaseSection from "../BaseSection/BaseSection";

function CareTeamMembersSection({ name, fields, excludedIds, errors = {}, onChangeField }) {
  const ids = useMemo(() => fields.ids.toJS(), [fields.ids]);
  const communityIds = useMemo(() => fields.communityIds.toJS(), [fields.communityIds]);

  const { data: careTeamMembers, isFetching: isFetchingCareTeamMembers } = useCommunityCareTeamMembersQuery(
    { communityIds },
  ...
  return (
    <div className="AddToGroupConversationForm-Section">
      <div className="AddToGroupConversationForm-SectionTitle">Community Care Team</div>
      ...
      <Row>
        <Col>
          <SelectField
            ...
            isDisabled={fields.communityIds.isEmpty() || isFetchingCareTeamMembers || isEmpty(careTeamOptions)}
            onChange={(name, value) => {
              const chatUserIds = filteredCareTeamMembers.filter((m) => value.includes(m.id)).map((m) => m.chatUserId);

              changeSelectField(name, value);
              changeSelectField("careTeamMembers.chatUserIds", chatUserIds);
            }}
