...
import { findWhere, first, reject } from "underscore";

import { SelectField } from "components/Form";

import { useSelectOptions } from "hooks/common";

import { useClientsQuery, useCommunitiesQuery, useOrganizationsQuery } from "hooks/business/conversations";
...
function ClientSection({
  ...
  const { data: clients, isFetching: isFetchingClients } = useClientsQuery(
    {
  ...
  const filteredClients = reject(clients, (o) => excludedContactIds.includes(o.associatedContactId));
  ...
  const onChangeClientField = useCallback(
    (name, value) => {
      ...
      const { associatedContactId, chatUserId } = findWhere(filteredClients, { id: value }) || {};

      console.log(findWhere(filteredClients, { id: value }));

      onClearField("client.careTeamMemberIds");
      onChangeFieldCb("client.associatedContactId", associatedContactId);
      onChangeFieldCb("client.chatUserIds", [chatUserId]);
    },
  ...
  return (
    <div className="AddToGroupConversationForm-Section">
      <div className="AddToGroupConversationForm-SectionTitle">Client</div>
      ...
      <Row>
        ...
        <Col md={4}>
          <SelectField
            ...
            label="Community"
            className="AddToGroupConversationForm-SelectField"
            isDisabled={!isInteger(fields.organizationId) || isFetchingCommunities}
            onChange={onChangeField}
        ...
        <Col md="4">
          <SelectField
            ...
            label="Client"
            className="AddToGroupConversationForm-SelectField"
            isDisabled={isFetchingClients || isEmpty(filteredClients) || !isInteger(fields.communityId)}
