...
import { findWhere, first, isNumber } from "underscore";

import { Button, Col, Form, Row, UncontrolledTooltip as Tooltip } from "reactstrap";

import { useAuthUser, useForm, useScrollable, useScrollToFormError, useSelectOptions } from "hooks/common";
...
function AddToConversationForm({ step, excludedContactIds, onClose, onChangeStep, onFieldChanged, onSubmitSuccess }) {
  ...
  const onChangeField = useCallback(
    (name, value) => {
      ...
      if (name === "clientId") {
        const { associatedContactId, chatUserId } = findWhere(clients, { id: value }) || {};
        changeField("contactId", associatedContactId);
        changeField("chatUserId", chatUserId);
      }
  ...
  const setDefaultOption = useCallback(
    (options, field) => {
      if (options.length === 1) {
        ...
        // 自动填充 chatUserId（副作用和 SelectField onChange 一致）
        if (field === "contactId") {
          const selectedContact = contacts.find((contact) => contact.id === first(options).value);
          const chatUserId = selectedContact ? selectedContact.chatUserId : undefined;
          onChangeField("chatUserId", chatUserId);
        }
        if (field === "clientId") {
          const { associatedContactId, chatUserId } = findWhere(clients, { id: first(options).value }) || {};
          onChangeField("contactId", associatedContactId);
          onChangeField("chatUserId", chatUserId);
        }
      }
    },
    [onChangeField, contacts, clients],
  );
