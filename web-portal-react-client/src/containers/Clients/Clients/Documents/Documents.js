...
import {
  useDocumentTemplateCountQuery
} from 'hooks/business/documents'

import {
  useESignRequestQuery,
  useCanAddESignRequestQuery
...
import EsignMultipleSignaturesPreview
  from "../../../Documents/e-sign/ESignDocumentTemplateEditor/ESignMultipleSignatures/EsignMultipleSignaturesPreview";
...
function Documents() {
  ...
  const filter = usePreparedClientDocumentFilterData(
    {clientId, ...state.filter.toJS()}
  )
  ...
  const {
    data: canAdd
  } = useCanAddClientDocumentsQuery({clientId})
  ...
  const {
    ...
    isFetching: isFetchingSignatureRequest
  } = useESignRequestQuery(
  ...
  const onViewCCD = useCallback(() => {
    window.open(path(
      `/clients/${clientId}/documents/ccd-${clientName.replace(' ', '_')}.xml`
    ))
  }, [clientId, clientName])
  ...
  const onRequestSignature = useCallback(document => {
  ...
  const onUploadSignatureRequestSuccess = useCallback(data => {
    refresh()
    updateSideBar()
    invalidateCount()
  ...
  return (
    <DocumentTitle title="Simply Connect | Clients | Client documents">
      <div className="Documents">
        ...
        <div className="Documents-Header">
          <div className="Documents-HeaderItem">
            <div className="Documents-Title">
                            <span className='Documents-TitleText'>
                                Documents
                            </span>
              <div className="d-inline-block text-nowrap">
                {clientName && (
                  <span className="Documents-ClientName">
                                        &nbsp;/&nbsp;{clientName}
                                    </span>
                )}
            ...
            <div className="Documents-Actions">
              ...
              <Filter
                className={cn(
                  ...
                  isFilterOpen
                    ? 'DocumentFilter-Icon_rotated_90'
                    : 'DocumentFilter-Icon_rotated_0'
                )}
        ...
        {
          showMultipleSignaturesPreview && <EsignMultipleSignaturesPreview
        ...
        {isSignatureRequestSuccessDialogOpen && (
          <SuccessDialog
            isOpen
            title={lastSignatureRequest?.ids?.length > 1 ? (
            ...
            buttons={[
              {text: 'Close', onClick: () => toggleSignatureRequestSuccessDialog()}
            ]}
          />
        )}
        ...
        {isReadyToSignByRequestDialogOpen && lastSignatureRequest?.ids?.length > 1 && (
          <SuccessDialog
            ...
            buttons={[
              {text: 'Close', onClick: () => toggleReadyToSignByRequestDialog()}
            ]}
          />
        )}

        {isReadyToSignByRequestDialogOpen && signatureRequest && (
          signatureRequest.recipientType === SELF
          || signatureRequest.notificationMethod === SIGN_NOW
        ) && (
          <DocumentReadyToSignDialog
            ...
            buttons={!signatureRequest.pinCode && [
        ...
        {isInstructionDialogOpen && (
          ...
          >
            <p>You can access Facesheet, Continuity of Care Document (CCD) and other documents uploaded by your health
              care providers.</p>
            <p>If a document requires your signature, you will receive a notification and will be able to sign it from
              here.</p>
          </Dialog>
        )}
        ...
        {isManagementInstructionDialogOpen && (
          ...
          >
            ...
            <p>If a document requires your signature, you will receive a notification and will be able to sign it from
              here.</p>
            <p>You can request a signature from Staff, Client or Parent/Guardian or Client Legal Representative if you
              have appropriate permission.</p>
          </Dialog>
        )}
