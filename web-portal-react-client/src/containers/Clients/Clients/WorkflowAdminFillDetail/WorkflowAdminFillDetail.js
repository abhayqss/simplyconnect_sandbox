import "./WorkflowAdminFillDetail.scss";
...
const WorkflowAdminFillDetail = (props) => {
  const { workflowId, clientId, fillClientId } = useParams();
  const location = useLocation();
  const searchParams = new URLSearchParams(location.search);
  const canEdit = searchParams.get("FN") === "true";
  ...
  const onCloseSuccessDialog = () => {
    ...
    if (shouldNavigate) {
      history.push(path(`/clients/${clientId}/workflow`));
    }
  };
  ...
  return (
    <>
      <div className="workflow-admin-fill-content">
...
// export default WorkflowAdminFillDetail;
export default connect(mapStateToProps, mapDispatchToProps)(WorkflowAdminFillDetail);
