      ...
      result.forEach((domain) => {
        domain?.needs?.forEach((need) => {
          need?.goals?.forEach((goal) => {
            ...
            if (goal.serviceStatusId === 1) {
              goal.serviceStatusId = 2;
            }

            if (goal.serviceStatusId) {
              const status = SERVICE_STATUS_CHOICES.find((item) => item.value === goal.serviceStatusId);
              if (status) {
                goal.serviceStatusTitle = status.text;
                // 将空格替换为下划线并转大写，例如 'In process' -> 'IN_PROCESS'
                goal.serviceStatusName = status.text.replace(/\s+/g, "_").toUpperCase();
              }
            }
          });
        });
