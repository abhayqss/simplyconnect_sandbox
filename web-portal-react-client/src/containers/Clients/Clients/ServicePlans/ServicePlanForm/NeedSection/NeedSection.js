...
class NeedSection extends PureComponent {
    ...
    constructor(props) {
        ...
        let optionMapper = { value: 'id', text: 'title' }

        this.getDomainSelectOptions = this.props.MemoizedSelectOptions(['servicePlan', 'domain'], optionMapper)
    ...
    onChangeField = (name, value, onCancel) => {
        this.changeField(
            name, trimStartIfString(value), onCancel
        )
    }
    ...
    onChangeGoalField = (index, name, value) => {
        this.props.onChangeGoalField(
            index, this.props.index, name, value
        )
    }

    onChangeGoalFields = (index, changes) => {
        this.props.onChangeGoalFields(
            index, this.props.index, changes
        )
    }
    ...
    render() {
        ...
        const programTypeFieldOpts = map(
            sortBy(where(programTypes, { domainId }), 'title'),
            o => ({ name: o.name, text: o.title, value: o.id })
        )

        const programSubTypeFieldOpts = map(
            sortBy(where(programSubTypes, { programTypeId }), 'title'),
            o => ({ name: o.name, text: `${o.title}, ${o.zcode}`, value: o.id })
        )
