...
import { last, map } from "underscore";
...
import { CheckboxField, DateField, PhoneField, RadioGroupField, SelectField, TextField } from "components/Form";

import { isEmpty, trimStartIfString } from "lib/utils/Utils";
...
class GoalSection extends Component {
  ...
  constructor(props) {
    ...
    this.getServiceStatusSelectOpts = memo(this.getServiceStatusSelectOpts);

    this.getServiceControlRequestStatusSelectOpts = memo(this.getServiceControlRequestStatusSelectOpts);
  }
  ...
  onChangeField = (name, value) => {
    ...
    this.props.onChangeField(this.props.index, last(name.split(":")), trimStartIfString(value));
  };

  onChangeFields = (changes) => {
    this.props.onChangeFields(this.props.index, changes);
  };

  onChangeDateField = (name, value) => {
    this.props.onChangeField(this.props.index, last(name.split(":")), value ? new Date(value).getTime() : null);
  };
  ...
  getDirectoryData() {
    return this.props.getDirectoryData({
      ...
      serviceControlRequestStatuses: ["service", "control", "request", "status"],
    });
  }

  getServiceControlRequestStatusSelectOpts() {
    const statuses = this.getDirectoryData().serviceControlRequestStatuses;
  ...
  getServiceStatusSelectOpts() {
    const statuses = this.getDirectoryData().serviceStatuses;
    // 移除 value 为 1 的选项
    return statuses
      .filter((o) => o.id !== 1)
      .map((o) => ({
  ...
  render() {
    const { index, fields, clientId, domainId, needIndex, programSubTypeId, canReviewByClinician } = this.props;

    const { serviceStatuses, serviceControlRequestStatuses } = this.getDirectoryData();
    ...
    return (
      <div className="GoalSection">
        <div className="GoalSection-Header">
          <div className="GoalSection-Title">Goal #{index + 1}</div>
          <Delete className="DeleteBtn" onClick={this.onDelete} />
        </div>
        <Row>
          <Col md={12}>
            <TextField
        ...
        <Row>
          <Col md={12}>
            <TextField
              ...
              renderLabelIcon={
                canReviewByClinician
                  ? () => <Info id="barriers-info-hint" className="GoalSection-SelectFieldLabelIcon" />
                  : noop()
              }
              tooltip={
                canReviewByClinician && {
        ...
        <div className="GoalSection-Header margin-top-15">
          <div className="GoalSection-Title">Service</div>
          {/*<Button
                        color={'success'}
        ...
        <Row>
          ...
          <Col md={4}>
            <SelectField
              ...
              options={this.getServiceControlRequestStatusSelectOpts(serviceControlRequestStatuses.length)}
              isMultiple={false}
              placeholder="Select"
          ...
          <Col md={4}>
            <SelectField
              ...
              value={fields.serviceStatusId === 1 ? 2 : fields.serviceStatusId}
              options={this.getServiceStatusSelectOpts(serviceStatuses.length)}
              isMultiple={false}
              placeholder="Select"
              className="GoalSection-SelectField"
              onChange={this.onChangeField}
...
export default connect(mapStateToProps)(withDirectoryData(GoalSection));
