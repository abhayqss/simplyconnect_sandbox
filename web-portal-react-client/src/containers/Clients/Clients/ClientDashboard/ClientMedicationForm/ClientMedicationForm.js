...
const ClientMedicationForm = (props) => {
  ...
  const [NDCOptions, setNDCOptions] = useState([{ text: "N/A", value: "N/A" }]);
  ...
  const findNdcName = (value) => {
    const data = medicationOptions.find((item) => item.value === value);
    setNDCOptions([data]);
    changeField("ndc", data.value);
  };
    ...
    if (value)
      service
        ...
        .catch((e) => {
          ...
          setShowErrorDialog(true);
          setErrorMessage("Failed to get the list");
        });
  };
    ...
    <>
      <Form className="ClientMedicationForm" onSubmit={onFormSubmit}>
        ...
        <div className="ClientMedication-Buttons">
          <Button outline color="success" disabled={isFetching} onClick={onCancelEditMedication}>
            Cancel
          </Button>
