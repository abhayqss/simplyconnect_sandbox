...
function ClientDashboard({
  ...
  if (isDataEmpty && isFetching) {
    ...
    const hideCreateAssessment = (() => {
      // 根据当前环境判断
      const environment = process.env.REACT_APP_SENTRY_ENVIRONMENT;

      if (environment === "sandbox" || environment === "localhost") {
        // 本地/sandbox环境，检查是否为2978
        return triggerCurrentOrgId === "2978";
      } else if (environment === "staging") {
        // stg环境，检查是否为657
        return triggerCurrentOrgId === "657";
    ...
    const permissions = {
      ...
      [CREATE_ASSESSMENT]: canAddAssessment && isActive && !hideCreateAssessment,
