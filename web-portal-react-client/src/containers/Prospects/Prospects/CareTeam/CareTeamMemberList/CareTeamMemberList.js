...
const COLUMNS = ["contactName", "organizationName", "communityName", "contacts", "@actions"];
...
function CareTeamMemberList({
  ...
  const [remove] = useCareTeamMemberDeletion(selected?.id);
  ...
  const confirmDeleteTitle = !selected?.isPrimaryContact
    ...
    : `The selected care team member is a primary contact for the prospect. ${selected?.contactName} will be deleted from the care team and will no longer be the primary contact for the prospect`;
    ...
    <>
      ...
      {isEditorOpen && (
        <CareTeamMemberEditor
          ...
          title={"prospect care team member"}
