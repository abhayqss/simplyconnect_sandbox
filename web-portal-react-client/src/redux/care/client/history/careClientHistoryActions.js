...
export function clear () {
    return { type: CLEAR_CARE_CLIENT_HISTORY }
}

export function clearError () {
    return { type: CLEAR_CARE_CLIENT_HISTORY_ERROR }
}

export function load (config) {
    return dispatch => {
        dispatch({ type: LOAD_CARE_CLIENT_HISTORY_REQUEST, payload: config.page })
        return service.find(config).then(response => {
            const { page, size } = config
            const { data, totalCount } = response
            dispatch({
                type: LOAD_CARE_CLIENT_HISTORY_SUCCESS,
                payload: { data, page, size, totalCount }
            })
        }).catch(e => {
            dispatch({ type: LOAD_CARE_CLIENT_HISTORY_FAILURE, payload: e })
        })
    }
}
