...
import InitialState from './CareClientCountInitialState'
...
export default function clientCountReducer (state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case CLEAR_CARE_CLIENT_COUNT:
            return state.removeIn(['error'])
                .setIn(['isFetching'], false)
                .setIn(['shouldReload'], action.payload || false)
                .removeIn(['value'])

        case CLEAR_CARE_CLIENT_COUNT_ERROR:
            return state.removeIn(['error'])

        case LOAD_CARE_CLIENT_COUNT_REQUEST: {
            return state.setIn(['error'], null)
                .setIn(['shouldReload'], false)
        }

        case LOAD_CARE_CLIENT_COUNT_SUCCESS:
            return state.removeIn(['error'])
                .setIn(['value'], action.payload)

        case LOAD_CARE_CLIENT_COUNT_FAILURE:
