...
export default function careTeamMemberDetailsReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case CLEAR_CARE_TEAM_MEMBER_DETAILS:
      return state.removeIn(["data"]).removeIn(["error"]).setIn(["isFetching"], false).setIn(["shouldReload"], false);

    case CLEAR_CARE_TEAM_MEMBER_DETAILS_ERROR:
      return state.removeIn(["error"]);

    case LOAD_CARE_TEAM_MEMBER_DETAILS_REQUEST:
      return state.setIn(["isFetching"], true).setIn(["shouldReload"], false).setIn(["error"], null);

    case LOAD_CARE_TEAM_MEMBER_DETAILS_SUCCESS:
      return state.setIn(["isFetching"], false).setIn(["data"], action.payload);
