...
import { ACTION_TYPES, NOTIFICATION_RESPONSIBILITY_TYPES } from "lib/Constants";
...
const { VIEWABLE, RESPONSIBLE, NOT_VIEWABLE } = NOTIFICATION_RESPONSIBILITY_TYPES;
...
export default function memberFormReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case CHANGE_CARE_TEAM_MEMBER_FORM_FIELD: {
      ...
      return state.changeField(field, value).updateHashCodeIf(shouldUpdateHashCode);
    }
    ...
    case LOAD_CARE_TEAM_MEMBER_DETAILS_SUCCESS: {
      ...
      return state
        .mergeDeep({
          fields: omit(action.payload, "notificationsPreferences"),
        })
        .setFetching(false)
    ...
    case LOAD_CARE_TEAM_NOTIFICATION_PREFERENCE_LIST_SUCCESS:
      return state.clearValidation().setFetching(false);
    ...
    case CHANGE_CARE_TEAM_MEMBER_NOTIFICATION_PREFERENCE: {
      ...
      const index = notificationsPreferences.findIndex((np) => np.eventTypeId === eventTypeId);
    ...
    case CHANGE_CARE_TEAM_MEMBER_ALL_NOTIFICATION_RESPONSIBILITIES: {
      return state.changeNotificationPreferences(
        state.fields.notificationsPreferences
          .map((np) => ({
            ...np.toJS(),
            responsibilityName:
    ...
    case CHANGE_CARE_TEAM_MEMBER_ALL_NOTIFICATION_CHANNELS: {
      const { excludeDisabled, excludeEditableAndWithChannels } = action.payload.options || {};
      const removeChannels = ["BLUE_STONE", "SECURITY_MESSAGE", "SMS"];

      return state.changeNotificationPreferences(
        state.fields.notificationsPreferences
          .map((np) => {
            const isNoChannels = [VIEWABLE, NOT_VIEWABLE].includes(np.responsibilityName);

            // 过滤出要移除的渠道
            let filteredChannels = action.payload.channels;
            // 需要根据 name 取出来
            if (np.eventTypeId === action.payload.missedMedicationReminderId) {
              filteredChannels = filteredChannels.filter((channel) => !removeChannels.includes(channel));
            }

            return {
              ...np.toJS(),
              channels: (np.canEdit ? !(excludeEditableAndWithChannels && isNotEmpty(np.channels)) : !excludeDisabled)
                ? !isNoChannels
                  ? filteredChannels // 使用过滤后的渠道
                  : []
                : np.channels,
            };
          })
    ...
    case VALIDATE_CARE_TEAM_MEMBER_FORM_FAIL: {
      ...
      return state.setValid(false).setFieldErrors(errors);
    }

    case VALIDATE_CARE_TEAM_MEMBER_FORM_SUCCESS:
      return state.setValid(true).clearValidation();
    ...
    case SAVE_CARE_TEAM_MEMBER_FAILURE: {
      return state.setError(action.payload).setFetching(false);
    }
