...
import InitialState from './CanViewCareTeamMemberInitialState'
...
export default function canAddServicePlanReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case CLEAR_CAN_VIEW_CARE_TEAM_MEMBER:
            return state.removeIn(['error'])
                .setIn(['isFetching'], false)
                .setIn(['shouldReload'], action.payload || false)
                .removeIn(['value'])

        case CLEAR_CAN_VIEW_CARE_TEAM_MEMBER_ERROR:
            return state.removeIn(['error'])

        case LOAD_CAN_VIEW_CARE_TEAM_MEMBER_REQUEST: {
            return state.setIn(['error'], null)
                .setIn(['shouldReload'], false)
        }

        case LOAD_CAN_VIEW_CARE_TEAM_MEMBER_SUCCESS:
            return state.removeIn(['error'])
                .setIn(['value'], action.payload)

        case LOAD_CAN_VIEW_CARE_TEAM_MEMBER_FAILURE:
