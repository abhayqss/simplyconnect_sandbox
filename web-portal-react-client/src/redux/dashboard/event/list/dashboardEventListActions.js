...
export function clear () {
    return { type: CLEAR_DASHBOARD_EVENT_LIST }
}

export function clearError () {
    return { type: CLEAR_DASHBOARD_EVENT_LIST_ERROR }
}


export function clearFilter () {
    return { type: CLEAR_DASHBOARD_EVENT_LIST_FILTER }
}

export function changeFilter (changes, shouldReload) {
    return {
        type: CHANGE_DASHBOARD_EVENT_LIST_FILTER,
...
export function load (config) {
    return dispatch => {
        dispatch({ type: LOAD_DASHBOARD_EVENT_LIST_REQUEST, payload: config.page })
        return service.findEvents(config).then(response => {
            const { page, size } = config
            const { data, totalCount } = response
            dispatch({
                type: LOAD_DASHBOARD_EVENT_LIST_SUCCESS,
                payload: { data, page, size, totalCount }
            })
        }).catch(e => {
            dispatch({ type: LOAD_DASHBOARD_EVENT_LIST_FAILURE, payload: e })
        })
    }
}
