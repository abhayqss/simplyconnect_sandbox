...
export function clear () {
    return { type: CLEAR_COMMUNITY_LOCATION_HISTORY }
}

export function clearError () {
    return { type: CLEAR_COMMUNITY_LOCATION_HISTORY_ERROR }
}

export function load (config) {
    return dispatch => {
        dispatch({ type: LOAD_COMMUNITY_LOCATION_HISTORY_REQUEST, payload: config.page })
        return service.find(config).then(response => {
            const { page, size } = config
            const { data, totalCount } = response
            dispatch({
                type: LOAD_COMMUNITY_LOCATION_HISTORY_SUCCESS,
                payload: { data, page, size, totalCount }
            })
        }).catch(e => {
            dispatch({ type: LOAD_COMMUNITY_LOCATION_HISTORY_FAILURE, payload: e })
        })
    }
}
