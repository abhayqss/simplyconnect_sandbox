...
export function clear () {
    return { type: CLEAR_COMMUNITY_LOCATION_LIST }
}

export function clearError () {
    return { type: CLEAR_COMMUNITY_LOCATION_LIST_ERROR }
}


export function clearFilter () {
    return { type: CLEAR_COMMUNITY_LOCATION_LIST_FILTER }
}

export function changeFilter (changes, shouldReload) {
    return {
        type: CHANGE_COMMUNITY_LOCATION_LIST_FILTER,
...
export function load (config) {
    return dispatch => {
        dispatch({ type: LOAD_COMMUNITY_LOCATION_LIST_REQUEST, payload: config.page })
        return service.find(config).then(response => {
            ...
            dispatch({
                type: LOAD_COMMUNITY_LOCATION_LIST_SUCCESS,
                payload: { data, page, size, totalCount }
            })
        }).catch(e => {
            dispatch({ type: LOAD_COMMUNITY_LOCATION_LIST_FAILURE, payload: e })
        })
    }
}
