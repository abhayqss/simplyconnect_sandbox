...
function DropzoneFieldNew(
  ...
  const onDrop = useCallback(acceptedFiles => {
    let files = [...value, ...acceptedFiles];

    if (files.length > maxCount) {
      files = files.slice(0, maxCount);
    }

    // 使用传递来的onChange方法来更新value
    onChange('vendorPhotos', files);
  }, [value, maxCount, onChange]);

  const onRemoveFile = file => {
    const updatedFiles = value.filter(o => o !== file);

    // 使用传递来的onChange方法来更新value
    onChange('vendorPhotos', updatedFiles);
  };
...
export default DropzoneFieldNew
