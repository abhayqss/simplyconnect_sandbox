...
import { ReactComponent as Publish } from "images/asset.svg"; // Using asset icon for publish
...
export default function Actions(props) {
  ...
  return (
    <div className={cn("Actions", className)}>
      ...
      {hasPublishAction && (
        <>
          <Publish id={"publish" + data.id} className="Actions-Item" onClick={onPublish} />
          <Tooltip
            placement="top"
            trigger="click hover"
            target={"publish" + data.id}
            modifiers={[
              {
                name: "offset",
                options: { offset: [0, 6] },
...
Actions.propTypes = {
  ...
  hasPublishAction: PropTypes.bool,
  ...
  publishHintMessage: PropTypes.string,
