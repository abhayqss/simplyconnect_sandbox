...
import { isInteger, getAddress, DateUtils as DU } from "lib/utils/Utils";

import { path } from "lib/utils/ContextUtils";

import { DemographicsDetails, AssociatedContactDetail } from "../../common";
...
export default function ClientDemographicsDetails({ data = {}, onViewContact, onCreateContact }) {
  ...
  return (
    <>
      ...
      <Detail title="UNIT #">{data.unit}</Detail>

      <Detail title="RISK SCORE">{data.riskScore}</Detail>

      <Detail title="CLIENT HAS AN ADVANCED DIRECTIVE ON FILE">{data.hasAdvancedDirectiveOnFile ? "Yes" : "No"}</Detail>

      <Detail title="OPT IN / OPT OUT CONSENT">{data.hieConsentPolicyTitle}</Detail>

      <Detail title="ADMIT DATE AND TIME">{map(data.admitDates, formatDate).join(", ")}</Detail>

      <Detail title="DISCHARGE DATE AND TIME">{map(data.dischargeDates, formatDate).join(", ")}</Detail>

      <Detail title="DEACTIVATE DATE">{data.deactivatedDate && formatDate(data.deactivatedDate)}</Detail>

      <Detail title="DEATH DATE AND TIME">{data.deathDateTime && formatDate(data.deathDateTime)}</Detail>
        ...
        {data.pharmacies ? (
          <div className="DemographicsDetail-Pharmacies">
            {data.pharmacies
              .filter((s) => !!s.trim())
              .map((pharmacy, index) => (
                <div className="DemographicsDetail-Pharmacy" key={index}>
                  {pharmacy}
                </div>
              ))}
          </div>
      ...
      {data.pharmacyPid && <Detail title="PHARMACY PID">{data.pharmacyPid}</Detail>}
      {data.emergencyContactName && <Detail title="PRIMARY EMERGENCY CONTACT">{ERdata()}</Detail>}
      ...
      <AssociatedContactDetail data={data} onViewContact={onViewContact} onCreateContact={onCreateContact} />
