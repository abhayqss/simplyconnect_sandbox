...
public class ResultsFactoryTest {
    ...
    public void testBuildingTemplate() {
        ...
        assertEquals("2.16.840.1.113883.10.20.22.2.3.1", section.getTemplateIds().get(0).getRoot());
        ...
        assertEquals(resultMock.getStatusCode().toLowerCase(), organizer.getStatusCode().getCode());
...
//        assertTrue(section.getText().getText().contains(resultObservationMock.getText()));
        assertEquals(resultObservationMock.getStatusCode().toLowerCase(), observation.getStatusCode().getCode());
