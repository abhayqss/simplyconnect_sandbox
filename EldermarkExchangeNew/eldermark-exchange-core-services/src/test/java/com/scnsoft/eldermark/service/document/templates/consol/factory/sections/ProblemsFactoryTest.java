...
public class ProblemsFactoryTest {
    ...
    public void testBuildingTemplate() {
        ...
        List<ProblemObservation> problemObservationMocks = new ArrayList<>();
        ...
        assertEquals("2.16.840.1.113883.10.20.22.2.5.1", section.getTemplateIds().get(0).getRoot());
        assertEquals("11450-4", section.getCode().getCode());
        assertEquals("2.16.840.1.113883.6.1", section.getCode().getCodeSystem());

        final List<ProblemConcernAct> problemActs = section.getConsolProblemConcerns();
        assertEquals(problems.size(), problemActs.size());

        // problem problem act
        ProblemConcernAct problemAct = problemActs.get(0);
        ...
        // problem observation
        List<Observation> problemObservations = getSubList(problemAct.getObservations(), org.openhealthtools.mdht.uml.cda.consol.ProblemObservation.class);
        ...
        var problemObservation = (org.openhealthtools.mdht.uml.cda.consol.ProblemObservation) problemObservations.get(0);
...
//        assertTrue(section.getText().getText().contains(String.format("<content ID=\"%s\">%s</content>", refId, problemObservationMock.getProblemName())));
        ...
        List<Observation> healthStatusObservations = getSubList(problemObservation.getObservations(), HealthStatusObservation.class);
        ...
        HealthStatusObservation healthStatusObservation = (HealthStatusObservation) healthStatusObservations.get(0);
        ...
        // problem status
        List<Observation> problemStatusObservations = getSubList(problemObservation.getObservations(), ProblemStatus.class);
        ...
        ProblemStatus problemStatusObservation = (ProblemStatus) problemStatusObservations.get(0);
