...
public class PayerFactoryTest {
    ...
    public void testBuildingTemplate() {
        ...
        for (int n = 0; n < 3; n++) {
            ...
            if (n != 2) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (i % 2 == 0)
                        policyActivity.setGuarantorCommunity(TestUtil.createCommunityMock());
                    else if (i != 3)
                    ...
                    if (i != 2)
                        policyActivity.setPayerCommunity(TestUtil.createCommunityMock());
        ...
        assertEquals("2.16.840.1.113883.10.20.1.9", payerSection.getTemplateIds().get(0).getRoot());
        ...
        for (Payer payerMock : payerList) {
            ...
            var policyActivityList = coverageActivity.getPolicyActivities();
            ...
            if (!CollectionUtils.isEmpty(payerMock.getPolicyActivities())) {
                ...
                for (PolicyActivity policyActivityMock : payerMock.getPolicyActivities()) {
                    var policyActivityCcd = TestUtil.getActById(policyActivityMock.getId(), policyActivityList);
                    ...
                    if (payerCommunityMock != null) {
                        ...
                        assertPerformerCommunities(payerCommunityMock, assignedEntity);
                    ...
                    var guarantorCommunity = policyActivityMock.getGuarantorCommunity();
                    Person guarantorPerson = policyActivityMock.getGuarantorPerson();

                    if (guarantorCommunity != null || guarantorPerson != null) {
                        ...
                        if (guarantorCommunity != null) {
                            assertPerformerCommunities(guarantorCommunity, assignedEntity);
                        } else {
                    ...
                    var participantMock = policyActivityMock.getParticipant();
...
//                            }
                    ...
                    var subscriberMock = policyActivityMock.getSubscriber();
...
//    }
    ...
    private void assertPerformerCommunities(Community communityMock, AssignedEntity assignedEntity) {
        assertEquals(communityMock.getId().toString(), assignedEntity.getIds().get(0).getExtension());
        if (!CollectionUtils.isEmpty(communityMock.getAddresses())) {
            TestUtil.assertAddresses(communityMock.getAddresses().get(0), assignedEntity.getAddrs().get(0));
        }

        if (communityMock.getTelecom() != null) {
            assertEquals(communityMock.getTelecom().getValue(), assignedEntity.getTelecoms().get(0).getValue());
        }

        if (communityMock.getName() != null) {
            assertEquals(communityMock.getName(), assignedEntity.getRepresentedOrganizations().get(0).getNames().get(0).getText());
        }
