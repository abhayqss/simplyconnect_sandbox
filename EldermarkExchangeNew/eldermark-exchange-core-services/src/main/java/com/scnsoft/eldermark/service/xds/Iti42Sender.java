...
public class Iti42Sender {
    ...
    private static final String SUBMIT_TEMPLATE_PATH = "xds/xds_submit_document_set.xml";
    private static final String APPROVE_TEMPLATE_PATH = "xds/xds_approve_document.xml";
    private static final String DEPRECATE_TEMPLATE_PATH = "xds/xds_deprecate_document.xml";
    ...
    public Iti42Sender(String repositoryUniqueId, String assigningAuthorityId, String registryUrl, SslConnectionFactory sslConnectionFactory) {
        ...
        try {
            InputStream fis = Thread.currentThread().getContextClassLoader().getResourceAsStream(SUBMIT_TEMPLATE_PATH);
            submitDocumentMessageTemplate = getStringFromInputStream(fis);
            fis = Thread.currentThread().getContextClassLoader().getResourceAsStream(APPROVE_TEMPLATE_PATH);
            approveDocumentMessageTemplate = getStringFromInputStream(fis);
            fis = Thread.currentThread().getContextClassLoader().getResourceAsStream(DEPRECATE_TEMPLATE_PATH);
    ...
    public Boolean sendDocumentMetadata(Iti42DocumentData data) throws IOException {
        ...
        String message = submitDocumentMessageTemplate
                ...
                .replaceAll(PLH_CREATION_TIME, data.getCreationTime() != null ? formatDate(data.getCreationTime()): "")
                ...
                .replaceAll(PLH_SUBM_TIME, Matcher.quoteReplacement(formatDate(Instant.now())))
    ...
    @SuppressFBWarnings(
        ...
        justification = "Connection is made to only one predefined URL"
    )
    // HTTP POST request
    protected void postMessage(String soapMessage) throws IOException {
        ...
        logger.info("Sending ITI-42 'POST' request to URL : {}", registryUrl);
        ...
        logger.info("ITI-42 response code : {}", responseCode);
    ...
    protected void checkXDSResponse(String responseStr) {

        logger.info("ITI-42 response : {}", responseStr);
        ...
        if (registryResponse == null) throw new XdsCommunicationException("Something goes wrong");
        ...
        if (!statusOk) {
            ...
            throw new XdsCommunicationException(codeContext);
        }

    }

    @SuppressFBWarnings("XXE_XMLSTREAMREADER")
    protected static OMElement xmlStringToOM(String inputString) {
    ...
    protected static String formatDate(Instant date) {
        return dateTimeFormatter.format(date.atZone(ZoneOffset.UTC));
    }
}
