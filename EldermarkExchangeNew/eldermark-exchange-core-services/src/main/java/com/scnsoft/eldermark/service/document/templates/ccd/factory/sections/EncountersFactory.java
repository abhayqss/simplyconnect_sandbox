 ...
 */
@Component
public class EncountersFactory extends OptionalTemplateFactory
        implements ParsableSectionFactory<EncountersSection, Encounter> {
    ...
    public org.eclipse.mdht.uml.cda.Encounter buildEncounterActivity(Encounter encounter) {
        ...
        if (CollectionUtils.isNotEmpty(encounter.getServiceDeliveryLocations())) {
            for (ServiceDeliveryLocation serviceDeliveryLocation : encounter.getServiceDeliveryLocations()) {
                ...
                participant2
                        .setParticipantRole(ccdSectionEntryFactory.buildServiceDeliveryLocation(serviceDeliveryLocation));
    ...
    public Act buildEncounterDiagnosis(com.scnsoft.eldermark.entity.document.ccd.ProblemObservation problemObservation) {
        ...
        entryRelationship.setObservation(
                ccdSectionEntryFactory.buildProblemObservation(problemObservation, Collections.<Class<?>>emptySet()));
    ...
    @Override
    public List<Encounter> parseSection(Client client, EncountersSection encountersSection) {
        ...
        for (Entry ccdEncounterEntry : encountersSection.getEntries()) {
            if (ccdEncounterEntry.getEncounter() != null) {
                ...
                encounter.setOrganization(client.getOrganization());
                ...
                if (!CollectionUtils.isEmpty(ccdEncounter.getParticipants())) {
                    ...
                    for (Participant2 ccdParticipant : ccdEncounter.getParticipants()) {
                        ...
                        serviceDeliveryLocations.add(sectionEntryParseFactory
                                .parseServiceDeliveryLocation(ccdParticipantRole, client, LEGACY_TABLE));
                    }
                ...
                if (!CollectionUtils.isEmpty(ccdEncounter.getEntryRelationships())) {
                    ...
                    for (EntryRelationship ccdEntryRelationship : ccdEncounter.getEntryRelationships()) {
                        ...
                        case RSON:
                            ...
                            indications.add(
                                    sectionEntryParseFactory.parseIndication(ccdObservation, client, LEGACY_TABLE));
                            break;
                        case COMP:
                            Act act = ccdEntryRelationship.getAct();
                            encounter.setProblemObservation(parseEncounterDiagnosis(act, client));
                            break;
                        default:
                            break;
                        }
                ...
                if (!CollectionUtils.isEmpty(ccdEncounter.getPerformers())) {
                    for (Performer2 ccdPerformer : ccdEncounter.getPerformers()) {
                        final Person person = performer2Factory.parsePerson(ccdPerformer, client.getOrganization(),
                                LEGACY_TABLE);
                        ...
                        encounterPerformer.setOrganization(client.getOrganization());
    ...
    private ProblemObservation parseEncounterDiagnosis(Act act, Client client) {
        ...
        for (EntryRelationship ccdEntryRelationship : act.getEntryRelationships()) {
            if (ccdEntryRelationship.getTypeCode() == x_ActRelationshipEntryRelationship.SUBJ) {
                problemObservation = sectionEntryParseFactory
                        .parseProblemObservation(ccdEntryRelationship.getObservation(), client, null);
            }
        }
