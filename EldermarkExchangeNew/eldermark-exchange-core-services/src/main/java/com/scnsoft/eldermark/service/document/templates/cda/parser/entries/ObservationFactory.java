...
public class ObservationFactory {
        ...
        if (CollectionUtils.isNotEmpty(observation.getEntryRelationships())) {
            ...
            for (EntryRelationship ccdEntryRelationship : observation.getEntryRelationships()) {
                if (ccdEntryRelationship.getObservation() != null) {
                    ...
                    severityObservations.add(parseSeverityObservation(ccdEntryRelationship.getObservation(), client,
                            legacyTable));
                }
                if (ccdEntryRelationship.getSubstanceAdministration() != null) {
                    ...
                    medications.add(substanceAdministrationsFactory.parseMedicationActivity(ccdEntryRelationship.getSubstanceAdministration(), client,
                            legacyTable));
                }
                if (ccdEntryRelationship.getProcedure() != null) {
                    ...
                    procedureActivities.add(procedureActivityFactory.parseProcedureActivity(ccdEntryRelationship.getProcedure(), client, legacyTable));
                }
    ...
    public ProblemObservation parseProblemObservation(Observation srcObservation, Client client, Problem targetProblem) {
    ...
    public AdvanceDirective parseAdvanceDirective(Observation srcObservation, Client client, String LEGACY_TABLE) {
        ...
        final Participant2Factory.ParticipantWrapper participantWrapper =
                Participant2Factory.parse(srcObservation.getParticipants(), client, LEGACY_TABLE);
        ...
        final List<AdvanceDirectiveDocument> documents = AdvanceDirectiveDocumentFactory.parseReferenceDocuments(
                srcObservation.getReferences(), advanceDirective, client);
