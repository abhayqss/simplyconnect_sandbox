...
public class Participant2Factory {
    ...
    public static ParticipantWrapper parse(EList<Participant2> srcParticipants, Client client, String LEGACY_TABLE) {
        ...
        for (Participant2 srcParticipant : srcParticipants) {
            ...
            case VRF:
                final Participant verifier = parseVerifier(srcParticipant, client, LEGACY_TABLE);
                participantWrapperVO.addVerifier(verifier);
                break;
            case CST:
                final Participant custodian = parseCustodian(srcParticipant, client, LEGACY_TABLE);
    ...
    private static Participant parseCustodian(Participant2 ccdParticipant, Client client, String LEGACY_TABLE) {
        ...
        if (ccdParticipant.getParticipantRole() != null) {
            ...
            if (CollectionUtils.isNotEmpty(ccdPlayingEntity.getNames())) {
                final Person person = CcdParseUtils.createPerson(ccdParticipant.getParticipantRole(),
    ...
    private static Participant parseVerifier(Participant2 ccdParticipant, Client client, String LEGACY_TABLE) {
        ...
        if (participantRole != null && participantRole.getPlayingEntity() != null) {
            ...
            if (CollectionUtils.isNotEmpty(ccdPlayingEntity.getNames())) {
                final Person person = CcdParseUtils.createPerson(participantRole, client.getOrganization(),
                        LEGACY_TABLE);
    ...
    public Participant parseCoverageTarget(Participant2 participant2, PolicyActivity policyActivity,
            Organization organization, String LEGACY_TABLE) {
        ...
        if (CcdParseUtils.hasContent(participantRole)) {
            ...
            if (playingEntity != null) {
                Person coverageTargetPerson = CcdParseUtils.createPerson(participantRole, organization, LEGACY_TABLE);
                participant.setPerson(coverageTargetPerson);
                policyActivity
                        .setParticipantDateOfBirth(CcdParseUtils.convertTsToDate(playingEntity.getSDTCBirthTime()));
            }
        }
    ...
    public Participant parseHolder(Participant2 participant2, PolicyActivity policyActivity, Organization organization,
            String LEGACY_TABLE) {
        final Participant participant = new Participant();
        participant.setOrganization(organization);
        participant.setLegacyTable(LEGACY_TABLE);
        ...
        if (CcdParseUtils.hasContent(participantRole)) {
            ...
            if (CollectionUtils.isNotEmpty(participantRole.getAddrs())) {
                final Person holderPerson = CcdParseUtils.createPerson(participantRole, organization, LEGACY_TABLE);
