...
public enum CdaDocumentType {
    ...
    CdaDocumentType(CCDAVersion ccdaVersion, TemplateId templateId) {
        this.ccdaVersion = ccdaVersion;
        this.templateId = templateId;
    }

    public static CdaDocumentType from(String root, String extension) {
        final TemplateId templateId = TemplateId.of(root, extension);
        for (CdaDocumentType type : values()) {
            if (CdaDocumentType.isIdentified(type) && type.getTemplateId().equals(templateId)) {
                return type;
            }
        }
    ...
    public static boolean isIdentified(CdaDocumentType documentType) {
