...
/**
 * Responsible for sending APNS Sandbox push notifications (both PushKit and UserNotifications) for all applications
 */
@Service
@Transactional
@ConditionalOnProperty(value = "apns.enableProd", havingValue = "true")
public class SansboxApnsPushNotificationSender extends BaseApnsPushNotificationSender {

    @Autowired
    public SansboxApnsPushNotificationSender(@Qualifier("sandboxApnsClient") ApnsClient apnsClient) {
        super(apnsClient);
    }
    ...
    protected PushNotificationRegistration.ServiceProvider resolveServiceProvider(PushNotificationVO pushNotificationVO) {
        return pushNotificationVO.getIosSettings().isVOIP() ?
                PushNotificationRegistration.ServiceProvider.APNS_PK_SB :
                PushNotificationRegistration.ServiceProvider.APNS_UN_SB;
    }
}
