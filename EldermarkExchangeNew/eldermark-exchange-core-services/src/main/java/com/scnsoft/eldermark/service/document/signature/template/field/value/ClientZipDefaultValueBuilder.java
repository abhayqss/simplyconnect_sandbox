...
@Component
public class ClientZipDefaultValueBuilder implements FieldDefaultValueBuilder {

    @Override
    public TemplateFieldDefaultValueType getTemplateFieldType() {
        return TemplateFieldDefaultValueType.CLIENT_ZIP;
    }
    ...
    public Object build(DocumentSignatureTemplateContext context) {
        return Optional.ofNullable(context.getClient())
                ...
                .flatMap(List::stream)
                .findFirst()
                .map(PersonAddress::getPostalCode)
                .orElse(null);
    }
}
