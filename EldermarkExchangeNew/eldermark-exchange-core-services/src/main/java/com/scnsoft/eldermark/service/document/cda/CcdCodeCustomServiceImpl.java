...
@Service
public class CcdCodeCustomServiceImpl implements CcdCodeCustomService {
    private final AnyCcdCodeDao anyCcdCodeDao;
    private final CcdCodeDao ccdCodeDao;

    @Autowired
    public CcdCodeCustomServiceImpl(AnyCcdCodeDao anyCcdCodeDao, CcdCodeDao ccdCodeDao) {
        this.anyCcdCodeDao = anyCcdCodeDao;
        this.ccdCodeDao = ccdCodeDao;
    }

    @Override
    public Optional<CcdCode> findOrCreate(String code, String displayName, CodeSystem codeSystem) {
        return findOrCreate(code, displayName, codeSystem.getOid(), codeSystem.getDisplayName());
    }

    @Override
    public Optional<CcdCode> findOrCreate(String code, String displayName, CodeSystem codeSystem, ValueSetEnum valueSet) {
        return findOrCreate(code, displayName, codeSystem.getOid(), codeSystem.getDisplayName(), valueSet.getOid());
    }

    @Override
    public Optional<CcdCode> findOrCreate(String code, String displayName, String codeSystemOid,
            String codeSystemName) {
        return findOrCreate(code, displayName, codeSystemOid, codeSystemName, null);
    }

    @Override
    public Optional<CcdCode> findOrCreate(String code, String displayName, String codeSystemOid, String codeSystemName,
            String valueSet) {
        ...
        return ccdCodeDao.findById(anyCcdCode.getId());

    }

    private AnyCcdCode createIfNotFound(String code, String displayName, String codeSystemOid, String codeSystemName,
            AnyCcdCode anyCcdCode) {
        if (anyCcdCode == null) {
//            logger.warn(
//                    "Can't find CcdCode for code={}, codeSystem={}. The unknown code will be added to the database.",
//                    code, codeSystemOid);
...
//                logger.warn(
//                        "CcdCode for code={}, codeSystem={} has a different displayName (expected='{}', actual='{}'). "
//                                + "The discrepant code will be saved as a separate entity.",
//                        code, codeSystemOid, anyCcdCode.getDisplayName(), displayName);
    ...
    private boolean isConcreteCcdCodeWithDifferentDisplayName(AnyCcdCode anyCcdCode, String displayName) {
        return anyCcdCode instanceof ConcreteCcdCode && displayName != null
                && !StringUtils.equalsIgnoreCase(anyCcdCode.getDisplayName(), displayName);
    }

    private UnknownCcdCode createUnknownCcdCode(String code, String displayName, String codeSystemOid,
            String codeSystemName) {
