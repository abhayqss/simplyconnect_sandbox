...
import com.scnsoft.eldermark.beans.reports.model.cstracking.CsTrackingReportClientRow;
import com.scnsoft.eldermark.beans.reports.model.cstracking.CsTrackingReportCommunityRow;
import com.scnsoft.eldermark.beans.reports.model.cstracking.CsTrackingReportInsuranceAuthorizationDto;
import com.scnsoft.eldermark.beans.reports.model.cstracking.CsTrackingReportRow;
...
import com.scnsoft.eldermark.dto.CsTrackingReportDto;
...
public class CsTrackingReportGenerator extends DefaultReportGenerator<CsTrackingReport> {

    private static final Logger logger = LoggerFactory.getLogger(CsTrackingReportGenerator.class);
    ...
    @Override
    public CsTrackingReport generateReport(InternalReportFilter filter, PermissionFilter permissionFilter) {
        logger.debug("Start generating CsTracking report");
        var report = new CsTrackingReport();
        populateReportingCriteriaFields(filter, report);
        report.setRows(createCsTrackingRows(filter, permissionFilter));
        return report;
    }

    private List<CsTrackingReportRow> createCsTrackingRows(InternalReportFilter filter, PermissionFilter permissionFilter) {
        ...
        var clientDetailsItems = clientDetails.stream().map(clientDetailsConverter::convert).collect(Collectors.toList());
        ...
        var clientHistoryDetails = clientHistoryData.stream().map(clientHistoryDetailsConverter::convert).collect(Collectors.toList());
        Optional.of(clientHistoryDetails).ifPresent(clientDetailsItems::addAll);
        ...
        var csTrackingReportDto = new CsTrackingReportDto();
        csTrackingReportDto.setClientHistoryData(clientDetailsItems);
        csTrackingReportDto.setCareTeamMembers(careTeamMembers);
        csTrackingReportDto.setAssessments(assessments);
        csTrackingReportDto.setNotes(notes);
        csTrackingReportDto.setEncounterNotes(encounterNotes);
        csTrackingReportDto.setEvents(events);
        csTrackingReportDto.setServicePlans(servicePlans);
        csTrackingReportDto.setClientInsuranceAuthorizations(clientInsuranceAuthorizations);

        var clientsByOrganization = new LinkedHashMap<Long, List<ClientDetailsAware>>();
        clientDetails.forEach(client -> clientsByOrganization.computeIfAbsent(
        ...
        return clientsByOrganization.values().stream()
                .map(clients -> createOrganizationRow(clients, csTrackingReportDto))
                .collect(Collectors.toList());
    }
    ...
    private CsTrackingReportRow createOrganizationRow(
            List<ClientDetailsAware> organizationClients,
            CsTrackingReportDto csTrackingReportDto
    ) {
        var row = new CsTrackingReportRow();

        var clientsByCommunity = new LinkedHashMap<Long, List<ClientDetailsAware>>();
        ...
        var communityRows = clientsByCommunity.values().stream()
                .map(clients -> createCommunityRow(clients, csTrackingReportDto))
                .collect(Collectors.toList());
    ...
    private CsTrackingReportCommunityRow createCommunityRow(
            List<ClientDetailsAware> communityClients,
            CsTrackingReportDto csTrackingReportDto
    ) {
        var row = new CsTrackingReportCommunityRow();
        row.setCommunityName(communityClients.get(0).getCommunityName());

        var clients = new LinkedHashMap<Long, List<ClientDetailsAware>>();
        ...
        var clientRows = clients.values().stream()
                .map(client -> createClientRow(client, csTrackingReportDto))
                .collect(Collectors.toList());
    ...
    private CsTrackingReportClientRow createClientRow(List<ClientDetailsAware> clients, CsTrackingReportDto csTrackingReportDto) {
        var row = new CsTrackingReportClientRow();
        var client = clients.get(0);
        var clientId = client.getId();

        row.setInsuranceName(client.getInNetworkInsuranceDisplayName());
        row.setClientCareCoordinatorName(csTrackingReportDto.getCareTeamMembers()
                .stream()
        ...
        var insuranceAuthorizations = Optional.ofNullable(csTrackingReportDto.getClientInsuranceAuthorizations()).stream()
                ...
                .map(insuranceAuthorization -> {
                    var insuranceAuthorizationDto = new CsTrackingReportInsuranceAuthorizationDto();
        ...
        fillClientHistoryData(row, clientId, csTrackingReportDto.getClientHistoryData());
        createAssessmentData(row, clientId, csTrackingReportDto.getAssessments());
        fillNotesData(row, clientId, csTrackingReportDto.getNotes());
        fillEncounterNotesData(row, clientId, csTrackingReportDto.getEncounterNotes());
        fillEventsData(row, clientId, csTrackingReportDto.getEvents(), csTrackingReportDto.getNotes());
        fillServicePlansData(row, clientId, csTrackingReportDto.getServicePlans());

        return row;
    }
    private void fillEventsData(
            CsTrackingReportClientRow row,
            Long clientId,
    ...
    private void fillEncounterNotesData(CsTrackingReportClientRow row, Long clientId, Map<Long, List<ClientEncounterNoteAware>> encounterNotes) {
        var clientEncounterNoteAwares = encounterNotes.get(clientId);
        if (clientEncounterNoteAwares != null) {
            row.setCsFaceToFaceFirstEncounterDate(clientEncounterNoteAwares.stream()
    ...
    private void fillNotesData(CsTrackingReportClientRow row, Long clientId, Map<Long, List<ClientNoteAware>> notes) {
    ...
    private void createAssessmentData(CsTrackingReportClientRow row, Long clientId, List<AssessmentDataAware> assessmentsData) {
        ...
        if (assessmentDataAwares != null) {
            ...
            var housingAssessments = assessmentsByName.get(Assessment.HOUSING);
            ...
            if (housingAssessments != null) {
                fillAssessmentData(row, Assessment.HOUSING, housingAssessments);
            }
    ...
    private void fillAssessmentData(CsTrackingReportClientRow row, String assessmentShortName, List<AssessmentDataAware> assessmentDataAwares) {
        switch (assessmentShortName) {
            ...
            case Assessment.NOR_CAL_COMPREHENSIVE:
                ...
                break;
            case Assessment.HOUSING:
                row.setLastHousingAssessmentCompleteDate(assessmentDataAwares.stream()
                        .filter(Objects::nonNull)
                        .map(assessment -> {
                            if (assessment.getDateCompleted() == null) {
                                return assessment.getLastModifiedDate();
                        ...
                        .max(Instant::compareTo)
                        .orElse(null));
                row.setTotalNumberCompletedHousingAssessment((long) assessmentDataAwares.size());
                break;
    ...
    private void fillClientHistoryData(CsTrackingReportClientRow row, Long clientId, List<ClientDetailsItem> clientHistoryData) {
    ...
    private void fillServicePlansData(CsTrackingReportClientRow row, Long clientId, List<ServicePlanDetailsAware> servicePlans) {
    ...
    private List<ClientHistoryDetailsAware> getClientHistoryData(InternalReportFilter filter, PermissionFilter permissionFilter, List<Long> clientIds) {
        var hasAccess = clientHistorySpecificationGenerator.hasAccess(permissionFilter);
        ...
        var specification = hasAccess.and(byUpdatedDateTimeIn.and(byClientIdIn));
    ...
    private Map<Long, List<ClientEncounterNoteAware>> getEncounterNotes(InternalReportFilter filter, PermissionFilter permissionFilter) {
        ...
        encounterNoteDao.findAll(specification, ClientEncounterNoteAware.class);
