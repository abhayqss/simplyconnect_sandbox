...
@Component
public class ClientAddressDefaultValueBuilder implements FieldDefaultValueBuilder {

    @Override
    public TemplateFieldDefaultValueType getTemplateFieldType() {
        return TemplateFieldDefaultValueType.CLIENT_ADDRESS;
    }
    ...
    public Object build(DocumentSignatureTemplateContext context) {
        return Optional.ofNullable(context.getClient())
                ...
                .flatMap(List::stream)
                .findFirst()
                .map(PersonAddress::getStreetAddress)
                .orElse(null);
    }
}
