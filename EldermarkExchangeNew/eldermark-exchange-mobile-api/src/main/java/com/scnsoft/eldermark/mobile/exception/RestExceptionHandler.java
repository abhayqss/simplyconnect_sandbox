...
public class RestExceptionHandler {
    ...
    protected ResponseEntity<Response> handleBusinessException(BusinessException ex) {
        ...
        return new ResponseEntity<>(Response.errorResponse(ex), Optional.ofNullable(ex.getHttpStatus()).map(HttpStatus::resolve).orElse(HttpStatus.INTERNAL_SERVER_ERROR));
    }
    ...
    protected ResponseEntity<Response> handleApplicationException(ApplicationException ex) {
        logger.error("Uncaught exception", ex);
        return new ResponseEntity<>(Response.errorResponse(new InternalServerException(ex.getMessage(), ex.getCode(), ex.getCause(), HttpStatus.INTERNAL_SERVER_ERROR)), HttpStatus.INTERNAL_SERVER_ERROR);
    }
