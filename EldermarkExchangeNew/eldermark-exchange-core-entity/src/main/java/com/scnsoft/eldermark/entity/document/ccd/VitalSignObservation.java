...
@NamedQueries({
        @NamedQuery(name = "vitalSignObservation. ", query = "select ccd.code, " +
                ...
                "INNER JOIN vso.resultTypeCode ccd WHERE vs.client.id IN :residentIds GROUP BY ccd.code"),
        @NamedQuery(name = "vitalSignObservation.getEarliestResidentVitalSign", query = "SELECT vso " +
                ...
                "WHERE vs.client.id IN :residentIds AND ccd.code = :ccdCode ORDER BY vso.effectiveTime ASC")
})
