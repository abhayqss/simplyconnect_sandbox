package com.scnsoft.eldermark.exchange.dao.source;

import org.springframework.stereotype.Repository;

import com.scnsoft.eldermark.exchange.Constants;
import com.scnsoft.eldermark.exchange.model.source.MedTimeCodeData;
import com.scnsoft.eldermark.framework.dao.source.StandardSourceDaoImpl;

@Repository(value = "medTimeCodeSourceDao")
public class MedTimeCodeSourceDaoImpl extends StandardSourceDaoImpl<MedTimeCodeData, Long> {
	
	protected MedTimeCodeSourceDaoImpl() {
		super(MedTimeCodeData.class, Long.class, Constants.SYNC_STATUS_COLUMN);
	}

}
